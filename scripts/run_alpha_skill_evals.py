...
import check_alpha_skill_evals as hardening
...
def run_command(
    ...
    try:
        result = hardening.run_command(arguments, cwd)
    except hardening.EvalCommandTimeout as error:
...
def load_cases(name: str, root: Path = ROOT) -> list[dict[str, Any]]:
    ...
    if name == "i-have-an-issue":
        ...
        runner_names = [case.get("runner") for case in cases]
        expected_runners = EXPECTED_RUNNERS[name]
        if (
            len(runner_names) != len(expected_runners)
            or any(not isinstance(runner_name, str) for runner_name in runner_names)
            or set(runner_names) != expected_runners
        ):
            raise EvalError(
...
def run_hardening_checks(client: str, pycc_binary: Path) -> None:
    failures = hardening.contract_failures(client)
    if client == "codex":
        failures.extend(hardening.runtime_failures(pycc_bin=pycc_binary))
        failures.extend(
            hardening.feedback_reproduction_failures(pycc_bin=pycc_binary)
        )
        failures.extend(hardening.behavioral_evidence_failures())
    if failures:
        raise EvalError("; ".join(failures))
...
def main() -> int:
    ...
    try:
        pycc_binary = arguments.pycc_bin.resolve()
        run_evals(
            arguments.client,
            pycc_binary,
        )
        run_hardening_checks(arguments.client, pycc_binary)
