...
python3 - \
  ...
  "$site_dir/language-support/index.html" \
  "$site_dir/diagnostics/index.html" <<'PY'
from html.parser import HTMLParser
import json
from pathlib import Path
import sys
...
PAGE_SPECS = {
    "language-support": {
        ...
        "required_visible_text": ("Python language support, with evidence.",),
    },
    "diagnostics": {
        ...
        "required_visible_text": ("Compiler diagnostics, exactly as emitted.",),
    },
    "status": {
        ...
        "required_visible_text": (
            ...
            "v0.4 — multi-file projects and incremental compilation — is in progress.",
...
python3 - \
  ...
  "$site_dir/ai-native/index.html" \
  "$site_dir/language-support/index.html" \
  "$site_dir/diagnostics/index.html" \
  "$canonical" \
  "$repo_root" <<'PY'
...
page_paths = [Path(argument) for argument in sys.argv[4:-2]]
canonical = sys.argv[-2]
...
expected_locations = {
    ...
    f"{canonical}language-support/",
    f"{canonical}diagnostics/",
}
locations = []
for entry in urls:
...
for required_link in (
    ...
    f"[Language support]({canonical}language-support/)",
    f"[Diagnostics]({canonical}diagnostics/)",
...
# 272 KiB aggregate ceiling and per-resource budgets are enforced without any
...
milestone_status = (
    "v0.4 is in progress. Cross-file project from imports have landed. "
    "Bare/submodule imports, namespace handling, broader project CLI behavior, "
    "and incremental compilation remain incomplete."
)
for surface in (llms, markdown):
    if milestone_status not in " ".join(surface.split()):
        raise SystemExit("LLM-readable files are missing the current v0.4 boundary")
