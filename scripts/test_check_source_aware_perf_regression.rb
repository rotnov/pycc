...
require "tmpdir"
require_relative "check_source_aware_perf_regression"
...
class PairedPerfRegressionTest < Minitest::Test
  ...
  def test_exits_zero_when_within_threshold_for_changed_executable_inputs
    Dir.mktmpdir do |dir|
      previous = estimates_file(dir, "previous.json", 1000.0)
      current = estimates_file(dir, "current.json", 1010.0)
      assert_equal 0, paired_main([current, previous, "false"])
    end
  end

  def test_exits_one_when_changed_executable_inputs_regress_beyond_threshold
    Dir.mktmpdir do |dir|
      previous = estimates_file(dir, "previous.json", 1000.0)
      current = estimates_file(dir, "current.json", 1030.0)
      assert_equal 1, paired_main([current, previous, "false"])
    end
  end

  def test_identical_executable_inputs_make_timing_delta_non_blocking
    Dir.mktmpdir do |dir|
      previous = estimates_file(dir, "previous.json", 1000.0)
      current = estimates_file(dir, "current.json", 1500.0)
      assert_equal 0, paired_main([current, previous, "true"])
    end
  end
  ...
  def test_respects_a_custom_threshold_argument
    Dir.mktmpdir do |dir|
      previous = estimates_file(dir, "previous.json", 1000.0)
      current = estimates_file(dir, "current.json", 1010.0)
      assert_equal 1, paired_main([current, previous, "false", "0.5"])
    end
  end
  ...
  def test_exits_two_when_executable_input_identity_is_missing_or_invalid
    Dir.mktmpdir do |dir|
      ...
      assert_equal 2, paired_main([current, previous, "unknown"])
    end
  end
  ...
  def test_exits_two_when_a_file_is_missing
    Dir.mktmpdir do |dir|
      ...
      assert_equal 2, paired_main([missing, previous, "false"])
    end
  end
  ...
  def test_exits_two_when_json_is_malformed
    Dir.mktmpdir do |dir|
      ...
      File.write(current, "{")
      assert_equal 2, paired_main([current, previous, "false"])
    end
  end
  ...
  def test_exits_two_when_threshold_is_negative_or_non_finite
    Dir.mktmpdir do |dir|
      previous = estimates_file(dir, "previous.json", 1000.0)
      current = estimates_file(dir, "current.json", 1000.0)
      assert_equal 2, paired_main([current, previous, "false", "-1"])
      assert_equal 2, paired_main([current, previous, "false", "NaN"])
    end
  end
end
