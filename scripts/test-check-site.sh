...
mutations = (
    ...
    (
        site_dir / "status" / "index.html",
        "The frontend performance gate runs independently",
        "The frontend performance gate follows the compiler work",
        "status page with stale performance-gate sequencing",
    ),
    (
        site_dir / "status" / "index.html",
        "fails on a regression greater than 2%",
        "records a regression greater than 2% without failing",
        "status page without the required regression gate",
    ),
    (
        site_dir / "status" / "index.html",
        "from its exact successful",
        "from an available recent",
        "status page without exact successful predecessor provenance",
    ),
    (
        site_dir / "status" / "index.html",
        "There is no missing-baseline",
        "A missing baseline starts a new",
        "status page with a reusable missing-baseline bootstrap",
    ),
    (
