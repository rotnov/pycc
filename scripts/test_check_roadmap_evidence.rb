...
class RoadmapEvidenceCliTest < Minitest::Test
  ...
  ACTIVE_D80_CONFORMANCE_ORACLE_WORKFLOW =
  ...
  def test_ignores_checked_items_hidden_in_fences_and_html_comments
    ...
    hidden_items.each do |hidden_item|
      stdout, stderr, status = run_checker(
        ...
        workflow: ACTIVE_D80_CONFORMANCE_ORACLE_WORKFLOW.read
      )
  ...
  def test_tier1_workflow_authorization_is_the_active_d80_digest
    assert_equal(
  ...
  def test_tier1_workflow_authorization_contains_only_active_d80
    assert_equal(
      [D80_CONFORMANCE_ORACLE_CI_WORKFLOW_SHA256],
  ...
  def test_d80_conformance_oracle_workflow_is_active_and_reviewed
    assert_equal(
      D80_CONFORMANCE_ORACLE_CI_WORKFLOW_SHA256,
    ...
    assert_equal(
      D80_CONFORMANCE_ORACLE_CI_WORKFLOW_SHA256,
      Digest::SHA256.file(ACTIVE_D80_CONFORMANCE_ORACLE_WORKFLOW).hexdigest
    )
    assert_equal(
      D80_CONFORMANCE_ORACLE_WORKFLOW_FIXTURE.read,
      ACTIVE_D80_CONFORMANCE_ORACLE_WORKFLOW.read
    )
    assert_equal(
    ...
    assert validate_source_aware_perf_gate_lifecycle(
      ACTIVE_D80_CONFORMANCE_ORACLE_WORKFLOW.read,
      ACTIVE_D80_CONFORMANCE_ORACLE_WORKFLOW.to_s
    )
  end
  ...
  def test_public_cli_accepts_the_active_d80_workflow
    stdout, stderr, status = run_checker(
  ...
  def test_public_cli_rejects_drift_in_the_active_d80_workflow
    workflow = D80_CONFORMANCE_ORACLE_WORKFLOW_FIXTURE.read.sub(
    ...
    refute status.success?
    assert_includes stderr, "does not match the reviewed active D-080 performance CI workflow"
  end
