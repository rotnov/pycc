...
# branch) is now in scope too (D-204, #769, Part 2 of #747), restricted to
...
# D-204, #769: `is not None` narrows the body -- `present + 1` reads
# `present` as a plain `int`, not `Optional[int]`.
if present is not None:
    print(present + 1)

# D-204, #769: `is None` narrows the `orelse` -- the mirror polarity.
if present is None:
    pass
else:
    print(present * 2)

# D-204, #769: a reassignment inside the narrowed branch kills the
...
def describe(x: int | None) -> None:
    # D-204, #769: the early-return continuation shape -- a guard clause
...
def local_optional() -> None:
    ...
    z: int | None = None
    print(z is not None)
    # D-204, #769: narrowing works identically inside a function body.
    if y is not None:
        print(y + 1)
