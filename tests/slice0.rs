...
#[test]
fn a_function_named_anything_other_than_main_is_a_codegen_error() {
    let dir = std::env::temp_dir().join(format!("pycc_e2e_badfn_{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    let src = write_fixture(&dir, "badfn.py", "def helper() -> None:\n    print(1)\n");
    let out = dir.join("badfn");
    ...
    let output = Command::new(pycc_bin())
        ...
        .unwrap();
    assert_eq!(output.status.code(), Some(1));
    assert!(String::from_utf8_lossy(&output.stderr).contains("helper"));
}
