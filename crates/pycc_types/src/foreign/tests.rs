...
/// An attribute load on a CPython object is the one operation Part 2 adds,
/// so it must *not* be refused -- including through the constraint solver,
/// which is what types the private helper's return in the second shape.
///
/// This is the positive half of the migration: Part 1's `reject_object_read`
/// refused both of these, and a regression that reinstated it would be
/// invisible to every refusal test below.
#[test]
fn an_attribute_load_on_a_cpython_object_is_admitted_in_both_producer_shapes() {
    for (shape, source) in both_producer_shapes("numpy.pi\n") {
        assert!(check_foreign(&source).is_none(), "{shape}: {source}");
    }
}

/// Every consuming site the refusal migration moved the `I0404` to, in both
/// producer shapes.
///
/// The ten condition entries are not redundant: `crate::lib`'s module-body
/// pass and its in-function pass each carry their own `if`, `while` and
/// three comprehension-guard call sites, five apiece, and only a per-site
/// case can prove none of the ten was missed.
#[test]
fn every_consuming_site_refuses_a_cpython_object_in_both_producer_shapes() {
    ...
    for (phrase, snippet) in [
        ...
        // The five in-function ones, which are a separate pass.
        (
            CONDITION,
            "def f() -> None:\n    if numpy.pi:\n        print(1)\n",
        ),
