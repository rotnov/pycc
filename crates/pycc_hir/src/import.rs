...
///
/// `definition_spans` is every top-level definition the module makes, with
/// its span, and never carries the import's own binding -- the import arm of
/// `module::lower_module_item` records nothing there -- so a name present in
/// both is exactly a second, non-foreign binding of it.
pub(crate) fn reject_shadowed_foreign_imports(
    ...
    imports
        ...
        .filter_map(|binding| match binding {
            ImportBinding::Foreign { local_name, .. } => Some(local_name),
            _ => None,
        })
        .filter_map(|local_name| {
            definition_spans
                ...
                .map(|(_, span)| {
                    // `C0001` by hand rather than through `unsupported`:
                    // the span is already a `Span` recorded during
                    // lowering, not an AST `TextRange`, and that helper
                    // takes only the range shape.
                    Diagnostic::error(
                        "C0001",
                        format!(
                            "`{local_name}` is bound both by a foreign `import` and by another \
                             top-level statement in this module; shadowing a foreign import is \
                             not supported yet"
                        ),
                        *span,
                    )
