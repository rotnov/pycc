...
// Subscripted-annotation lowering (`ClassName[...]`, PEP 560 #611/#693,
// D-228 containers, #931 non-class bases) lives in its own child module so
// this file does not keep growing (#663). A child module sees this module's
...
/// #913: a `ClassVar` named after a method the dataclass *synthesizes*.
/// CPython's `dataclasses` would keep the class attribute and skip the
/// synthesis; pycc synthesizes unconditionally, so the spelling is rejected.
/// All three names are exercised because a single or-pattern arm is not
/// reliable evidence that every arm's region is covered in this crate
/// (see `class.rs`'s own note on the `slot_ty_from_init_rhs` match).
#[test]
fn a_class_var_named_after_a_synthesized_dataclass_method_is_rejected() {
    assert_capability_error_message(
        "@dataclass\nclass C:\n    x: int\n    __repr__: ClassVar[int] = 8\n",
        "a `ClassVar` named `__repr__` is not allowed in a `@dataclass` body",
    );
    assert_capability_error_message(
        "@dataclass\nclass C:\n    x: int\n    __init__: ClassVar[int] = 8\n",
        "a `ClassVar` named `__init__` is not allowed in a `@dataclass` body",
    );
    assert_capability_error_message(
        "@dataclass\nclass C:\n    x: int\n    __eq__: ClassVar[int] = 8\n",
        "a `ClassVar` named `__eq__` is not allowed in a `@dataclass` body",
    );
}
