...
/// The base of a foreign attribute load or method call, or the callee of a
/// direct call, as a `PyObject *`.
///
...
/// arm), and `MirExpr::ObjCall` only where the callee's is (#1313), and `ty_to_basic_type` maps that to a pointer, so every other
...
pub(super) fn expect_object_pointer(scalar: Scalar<'_>) -> PointerValue<'_> {
    let Scalar::Object(ptr) = scalar else {
        panic!(
            "pycc_codegen: internal error: a foreign attribute base or call callee did not \
             evaluate to a CPython object -- pycc_mir lowers `ObjAttrGet`/`ObjMethodCall`/\
             `ObjCall` only for a `Ty::Object` base or callee"
        )
    };
    ptr
}
