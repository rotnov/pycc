                ...
                let ctor = class_def
                    ...
                    .unwrap_or_else(|| {
                        panic!(
                         ...
                         lowers through `lower_class`, by inheritance or by synthesis, and a \
                         call to an enum class never reaches MIR: `pycc_hir::class::enum_call` \
                         and `pycc_types::class::resolve_instantiation` reject it with `C0001` \
                         (#921)"
                        )
                    });
