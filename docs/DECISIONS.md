...
| D-021 | v0.1 may embed exactly the two protocol-output C helpers documented in `RUNTIME.md`; LLVM emits the native object, then Clang compiles the C bootstrap and links both for the same native triple (using LLD for MSVC), while the full heap/object runtime remains pure Rust | accepted |
...
## D-021: Target-aware C bootstrap exception for v0.1

- Status: accepted (PR-1/PR-2 depends on the exception)
- Context: the v0.1 vertical slice must emit exact protocol bytes before the full `pycc_rt` heap/object runtime exists. Rust's standard output path adds platform behavior that is outside this deliberately tiny bootstrap contract, while the generated LLVM object and its output helpers must still agree on the host-native target ABI. Explicit cross-target selection is deferred until the planned `--target` wiring.
- Decision: v0.1 embeds exactly the two C helpers documented in `RUNTIME.md` (`pycc_rt_print_i64` and `pycc_rt_name_error`). `pycc_codegen` emits the program object directly with LLVM's native `TargetMachine`; the link step independently obtains LLVM's native target triple, passes it to a Clang-compatible driver to compile the embedded C source, and links that runtime object with the already-emitted program object. MSVC targets additionally select bundled LLD. This is a narrow bootstrap exception only: the full heap/object runtime remains a pure-Rust static library with a `#![no_std]`-friendly core.
- Alternatives: use host `cc` defaults (rejected because host and target ABIs can diverge); route bootstrap output through Rust `std::io` (rejected because it expands the temporary runtime surface and does not preserve the exact-byte contract as directly); implement the full runtime before the first vertical slice (rejected because it blocks an end-to-end compiler proof on unrelated runtime scope).
- Consequences: adding any C helper beyond those two requires a new decision. Native object emission and Clang compilation/linking must remain locked to the same LLVM-reported native triple in tests and CI. Adding `--target` requires threading one explicit target through both paths rather than treating this native-only bootstrap as cross-compilation support. Once the Rust runtime can provide the same exact-byte protocol on every Tier-1 target, a new ADR supersedes this exception and removes the embedded C source.
