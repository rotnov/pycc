...
- **What is still an error, now `T0003`.** Any position with no inferable
  ...
  the attribute and the class. The unannotated `self.d = {}` and a
  tuple-unpacking target (`L, R = [], []`) are *not* part of this: both are
  rejected earlier with `C0001`, the first until its `self.d[k] = v` producer
  exists ([#891](https://github.com/rotnov/pycc/issues/891)). An empty tuple
