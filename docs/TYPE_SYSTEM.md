...
4. No implicit `Optional`, no implicit numeric narrowing **or widening** (D-086 — includes `int` at a `float`-annotated boundary; `float(x)` for `x: int | float | bool` is now a real callable builtin conversion, D-086's own remedy for that boundary — `int(...)` remains unimplemented and out of scope), no untyped containers (`x = []` requires inferable or annotated element type).
