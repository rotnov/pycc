...
A base class naming one of eleven subclassable builtin types (`int`, `float`, `str`, `bytes`, `bytearray`, `list`, `tuple`, `dict`, `set`, `frozenset`, `complex`; `object` and the builtin types CPython refuses as a base are not among them) is not supported yet and is reported as `C0001` "class `X` inherits from builtin type `T`" rather than as an unknown class, unless the module rebinds that name earlier through a type alias, import, `def`, or top-level binding that itself lowered, in which case the unknown-class text stands (Part 1 of [#1283](https://github.com/rotnov/pycc/issues/1283)); hosting a `frozenset` subclass is tracked by [#1319](https://github.com/rotnov/pycc/issues/1319).
