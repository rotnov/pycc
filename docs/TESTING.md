...
Run `cargo fmt --all -- --check` before every pull request. CI installs the
`rustfmt` component for the pinned toolchain and rejects formatting drift
before compiling or testing the workspace.
