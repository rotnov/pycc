...
token normalization, and Setext level-2 headings are recognized through
blockquote and list container prefixes. Because this evidence file is a data
ledger rather than general documentation, fenced and raw-HTML blocks are
rejected fail-closed anywhere in the document; they cannot turn the canonical
table into rendered code or an unaudited HTML surface. The checkpoint schema
also requires a non-boolean JSON integer version before accepting version `1`.
This staging
