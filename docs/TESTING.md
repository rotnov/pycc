...
- **Practical notes on what actually shows up as a coverage gap** (learned building the first few v0.1 crates — verified directly against `cargo llvm-cov`'s HTML report, not assumed):
  ...
  - **The per-file summary table sums regions across *every compilation* of a crate, so a line reachable only through the `pycc` binary still reads as missed.** A crate in this workspace is compiled at least twice — once plainly (linked into the `pycc` binary and therefore into every integration test under `tests/`) and once with `--cfg test` for its own unit-test binary. Those are two independent region sets over the same source. Code that only the integration suite drives is covered in the first set and missed in the second, and the summary reports the sum, so the file shows below 100% even though every line ran. Diagnosed exactly this way on [#603](https://github.com/rotnov/pycc/issues/603): new `HirExpr::UnaryOp` arms in `pycc_hir`, `pycc_mir`, and `pycc_types` were exercised end to end by `tests/issue_603_unary_general_operand.rs` but by no inline `mod tests` case, and `build-test-coverage` failed at 99.93% regions. **The merged views cannot show this** — `--show-missing-lines`, `--lcov`, and `report --text` without `--show-instantiations` all report the *union* across compilations and therefore show the lines as covered; only the per-function `regions` array in `cargo llvm-cov report --workspace --json` (entries whose execution count is `0`, aggregated per source range across all instantiations) identifies them, and `nm` on `target/llvm-cov-target/debug/pycc` versus `target/llvm-cov-target/debug/deps/<crate>-<hash>` confirms the two distinct symbol copies. Fix: add inline unit tests in the crate's own `mod tests` for any arm an integration test is the only thing reaching. Reach for the JSON regions view first whenever the summary and the missing-lines view disagree, instead of re-running the merged views in different formats.
