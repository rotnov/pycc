...
| `I0403` | error | CPython import the effective interop policy admits but this build cannot embed (a non-standard-library root until #1225, an excluded Tcl/Tk root, a `--target` build, or a Windows host until #1226); `pycc build --ext` is required |
