...
|---|---|---|
| `C0001` | error | valid Python construct is not implemented by this pycc version -- e.g. `hash()` of a type CPython hashes but this version does not yet, "`hash()` of `<ty>` is valid Python but not implemented yet" (#1331), including a user-class instance whose hash pycc does not compile yet, with a help line naming the reason (#1335) |
| `C0002` | error | recognized stdlib module, but the specific imported symbol is not registered (D-136), e.g. `from math import isnan` |
| `C0003` | error | `--ext` cannot carry the signature of a public function, of a public `@staticmethod`/`@classmethod` of a public class, or of a public instance method of a public class that some *published, constructible* class reaches through its method resolution order and whose method name that class resolves to the declaring class rather than to a binding of its own, across the CPython boundary (D-244): `docs/RUNTIME.md`'s `ext` boundary section is the canonical admissibility matrix for what crosses and in which direction, and for which members are in the export set at all, so one unexportable signature fails the whole `--ext` build -- rename the member with a leading underscore (D-038) to drop it from the export set, or build without `--ext` |
| `L0001` | error | syntax error (span + expected set) *(also reused, without an "expected set", for a post-parse context violation caught during HIR lowering -- `break`/`continue` outside a loop, `async for` outside an async function (D-148), `yield`/`yield from` outside a function (D-149), `return`/`break`/`continue` inside a `finally` block that would exit it (PEP 765, D-193, Part 1 of #543), `return`/`break`/`continue` inside an `except*` clause body (PEP 654, #795) -- each of these also when hidden behind an `if`/`elif TYPE_CHECKING:` guard, whose folded-away body is re-walked for context violations only (#905, `crates/pycc_hir/src/stmt/type_checking.rs`) -- and a `from __future__ import ...` CPython rejects -- an unknown feature name including `*` (`future feature <name> is not defined`), `braces` (`not a chance`), or a future import after the docstring-and-future-imports prologue (`from __future__ imports must occur at the beginning of the file`) (#919, D-229) -- since CPython classifies all of these as `SyntaxError` (a `SyntaxWarning` as of CPython 3.14 for the `finally` case specifically) too; `T0024` ("`return` outside a function") is the closest existing precedent for this same "keyword valid only in a specific context" family but lives under `T0xxx` instead, an accepted, deliberately unfixed inconsistency -- see D-148)* |
...
| `T0033` | error | value does not support list/dict/set/frozenset operations (subscript, slicing, item assignment, `for`, `.append()`, `.pop()`, `.get()`, `.add()`, `len()`), or `len()` called with the wrong number of arguments |
...
| `T0038` | error | set codegen only supports `set[int]` in v0.2 (D-122) — gates both an inferred set-literal element type and a written `set[T]` annotation (D-228), and a written `frozenset[T]` annotation, whose only compiled form is `frozenset[int]` (Part 1 of #1319) |
...
| `T0044` | error | class has no attribute or method with the given name (D-154, Part 1 of #375); also a store to an attribute that cannot be written: a class-level attribute (#911), a property without a setter (#377), or an enum member's `value`/`name` ([#1219](https://github.com/rotnov/pycc/issues/1219)); a mismatched instantiation/method call-site argument list reuses `T0021` instead, exactly like an ordinary function call; also reported (PEP 560, #610/#611) when `ClassName[...]` is used **in value position** on a class that defines no `__class_getitem__` anywhere in its MRO and is not a PEP 695 generic class, matching CPython's `TypeError: type 'C' is not subscriptable` — annotation position is no longer gated on subscriptability, because PEP 649/749 evaluates annotations lazily ([#1130](https://github.com/rotnov/pycc/issues/1130)); and ([#931](https://github.com/rotnov/pycc/issues/931)) when the base of a subscripted annotation resolves to something that is not a class at all — a PEP 695 type parameter (`T[int]`), a builtin scalar (`int[str]`), `Self` inside a class, or a `type` alias to a non-class type other than the buffer carrier (`type Arr = memoryview` then `Arr[float]` resolves to the carrier and is accepted, #1130) — matching the same CPython `TypeError` instead of silently discarding the type argument; an alias to a class behaves as that class, an undefined base keeps `C0001`, and `Any[...]` keeps `T0002` |
| `T0045` | error | cannot reassign a `Final` name (PEP 591, #383) — variable-level annotations only (module-level and function-local); `Final` on parameters is out of scope, and a class-body `Final` attribute (#916) is accepted but rejected on write by `T0044` instead |
...
| `T0047` | error | `super()` does not proxy instance attributes (#587) — an attribute established by `self.<attr> = ...` is reachable only through the method's own receiver, matching CPython's `super` object, which proxies class-level attributes and descriptors along the MRO but not the instance's own attributes |
