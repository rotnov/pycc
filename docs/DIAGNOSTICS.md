...
| `C0003` | error | `--ext` cannot carry a public function's signature across the CPython boundary (D-244): `docs/RUNTIME.md`'s `ext` boundary section is the canonical admissibility matrix for what crosses and in which direction, and rule 1 exports every public module-level function, so one unexportable signature fails the whole `--ext` build -- rename it with a leading underscore (D-038) to drop it from the export set, or build without `--ext` |
...
| `I0405` | error | a buffer-typed signature in native mode, under either source spelling (`memoryview` or `ndarray`); `pycc build --ext` is required |
