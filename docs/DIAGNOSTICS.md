...
| `C0003` | error | `--ext` cannot carry a public function's signature across the CPython boundary (D-244): Part 1 of #1025 bridges `int` parameters and `int` results only, and rule 1 exports every public module-level function, so one unexportable signature fails the whole `--ext` build -- rename it with a leading underscore (D-038) to drop it from the export set, or build without `--ext` |
