...
| `C0003` | error | `--ext` cannot carry a public function's signature across the CPython boundary (D-244): the boundary carries `int`, `float` and `bool` in either direction and `None` as a return type only (#1036, #1048) -- `docs/RUNTIME.md`'s `ext` boundary section is the canonical admissibility matrix -- and rule 1 exports every public module-level function, so one unexportable signature fails the whole `--ext` build -- rename it with a leading underscore (D-038) to drop it from the export set, or build without `--ext` |
