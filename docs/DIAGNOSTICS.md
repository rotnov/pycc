...
| `I0403` | error | CPython import the effective interop policy admits but this build cannot embed (an excluded Tcl/Tk root, a `--target` build, or a root outside the standard library on a Windows host until #1287; such a root embeds from `pycc.lock` on macOS and Linux since #1242, and a Windows host embeds standard-library roots since #1286); `pycc build --ext` is required |
