...
| `T0052` | error | cross-MRO attribute redeclaration with a differing declared type (issue #676, D-209) — two distinct classes in one class's own C3-linearized MRO each declare an instance attribute of the same name with a different type; rejected at class-definition time since pycc has no per-instance runtime type tag to safely coerce at the shared, statically-dispatched assignment site |
