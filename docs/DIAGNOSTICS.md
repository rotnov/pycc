...
| `C0003` | error | `--ext` cannot carry the signature of a public function, of a public `@staticmethod`/`@classmethod` of a public class, or of a public instance method of a public class whose methods some *published, constructible* class's method resolution order reaches, across the CPython boundary (D-244): `docs/RUNTIME.md`'s `ext` boundary section is the canonical admissibility matrix for what crosses and in which direction, and for which members are in the export set at all, so one unexportable signature fails the whole `--ext` build -- rename the member with a leading underscore (D-038) to drop it from the export set, or build without `--ext` |
