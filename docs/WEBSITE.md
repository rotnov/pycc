...
| Public-CLI controls against the real record (nested-field mirror, corrupted or missing stage artifact, missing trace record, rewritten verification test, drifted test names, drifted verification command, state overstated and understated in the record and on the page, drifted Markdown and LLM markers, softened limitations, a paraphrased excerpt, a moving branch link, and the positive control that the shipped record is accepted) | `scripts/site_pipeline_evidence_test.py` run by `scripts/test-check-site.sh` (Pages only; the name is deliberately not `test_*`) | must pass |
