...
| [585](https://peps.python.org/pep-0585/) | Builtin generics: `list[int]` (D-105), `dict[str, int]`/`set[int]` (D-121/D-122, PR-11a), and `tuple[...]` (D-115/D-116, PR-11b) all ship real codegen, and since D-228 (issue #918) the parameterized *annotation* forms `list[T]`, `set[T]`, `dict[K, V]` and `tuple[A, B, ...]` are lowered in parameter, local- and module-variable and type-alias positions, with explicit arity validation (`T0053`) and the same element-type capability gates the literals run (`T0034`/`T0036`/`T0038`/`T0039`). Passing a container *value* across a function boundary now works from real Python source when the parameter carries such an annotation — measured end to end for all four families against CPython 3.14. Still deferred: container types in **return** position (issue #925, which is also what keeps D-146's private-helper solver reaching an unhandled codegen case, issue #926), a **protocol attribute's** type (no class can declare a container-typed attribute slot, so such an attribute is unsatisfiable; a protocol *method's* parameter does lower), nested containers (#895), unions inside containers (#747), `frozenset[...]`/`type[...]` (no `Ty` variant under D-109's 16-byte ceiling), the variadic `tuple[int, ...]` and empty `tuple[()]` (both rejected with `T0053`), and inferring an empty literal's element type from its annotation (#927) | typing | `pep_0585_builtin_generics.py`, `dict_order.py`, `pep_0585_set_int.py`, `tuple_heterogeneous.py` | ◐ |
