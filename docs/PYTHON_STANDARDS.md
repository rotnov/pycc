...
| PEP | Feature | Cat | Test | St |
|---|---|---|---|---|
| [604](https://peps.python.org/pep-0604/) | Union syntax, e.g. `int` or `str`. `Optional[int]` (`T` or `None`, either operand order) type-checks and codegens end-to-end -- declared, assigned, returned from a function, module-global and function-local, tested for presence with `is`/`is not None` (D-197, [#763](https://github.com/rotnov/pycc/issues/763), Part 1 of [#747](https://github.com/rotnov/pycc/issues/747)); flow-sensitive narrowing of an `Optional[int]` back to plain `int` inside a top-level `is`/`is not None` branch, including the early-return continuation shape (D-201, [#769](https://github.com/rotnov/pycc/issues/769), Part 2 of [#747](https://github.com/rotnov/pycc/issues/747)). Not proven: a general union with neither side `None` or a 3+-operand chain (`T0048`); `Optional[T]` for any inner type other than `int` (`T0049`); narrowing through a compound `and`/`or` test or narrowing a name to `None` | typing | `pep_0604_union.py` | ◐ |
