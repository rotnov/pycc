...
| PEP | Feature | Cat | Test | St |
|---|---|---|---|---|
| [604](https://peps.python.org/pep-0604/) | Union syntax, e.g. `int` or `str`. `Optional[T]` for `T` in `{int, float, bool}` (`T` or `None`, either operand order) type-checks and codegens end-to-end -- declared, assigned, returned, module-global and function-local, tested with `is`/`is not None` (D-197, [#763](https://github.com/rotnov/pycc/issues/763), Part 1 of [#747](https://github.com/rotnov/pycc/issues/747); widened from `int`-only to also accept `float` and `bool` by [#809](https://github.com/rotnov/pycc/issues/809), Part 3 of #747); flow-sensitive narrowing back to plain `T` inside a top-level `is`/`is not None` branch, including the early-return continuation shape (D-201, [#769](https://github.com/rotnov/pycc/issues/769), Part 2 of #747). Not proven: a general union with neither side `None` or a 3+-operand chain (`T0048`); `Optional[T]` for any inner type outside `{int, float, bool}` -- notably `Optional[str]` and any refcounted/pointer inner type (`T0049`); narrowing through a compound `and`/`or` test or narrowing a name to `None` | typing | `pep_0604_union.py` | ◐ |
