...
12. PEP 654 (`except*`/`ExceptionGroup`) and PEP 572 (walrus `:=`) were
    ...
    comprehension-scope-skip semantics per #774's recorded scope cut. Those are
    `core` gaps under [D-177](./decisions/D-177-scope-matrix-acceptance-to-proven-semantics.md), and a `core` gap
    forces `◐` -- including PEP 654's six [D-202](./decisions/D-202-pep-654-except-star-and-exceptiongroup.md)
    simplifications, which D-202 records as independently revisitable rather
    than a permanent non-goal. #795's `return`/`break`/`continue` gap is the
    exception: pycc matches the oracle, so the manifest records `out-of-scope`.
13. PEP 563 (`from __future__ import annotations`) follows rule 9's pattern:
    [#919](https://github.com/rotnov/pycc/issues/919) (D-229) accepts the
...
| [585](https://peps.python.org/pep-0585/) | Builtin generics: `list[int]` (D-105), `dict[str, int]`/`set[int]` (D-121/D-122, PR-11a), and `tuple[...]` (D-115/D-116, PR-11b) all ship real codegen, and since D-228 (issue #918), widened by issue #925, the parameterized *annotation* forms `list[T]`, `set[T]`, `dict[K, V]` and `tuple[A, B, ...]` are lowered — joined by `frozenset[T]` since #1326 (Part 1 of #1319), with `frozenset()`/`frozenset(x)` construction from a `set[int]`, `frozenset[int]` or `list[int]` — in parameter, return, local- and module-variable and type-alias positions, with explicit arity validation (`T0053`) and the same element-type capability gates the literals run (`T0034`/`T0036`/`T0038`/`T0039`). Passing a container *value* across a function boundary now works from real Python source in both directions — an annotated parameter (#918) and an annotated return type (#925) — measured end to end for all five families against CPython 3.14 (`frozenset[int]`: `tests/issue_1326_frozenset.rs`). Still deferred: a **protocol attribute's** type (not supported yet; a protocol *method's* parameter and return type both lower), a container-typed instance attribute other than a `list[int]`/`dict[str, int]` seeded from an `__init__` parameter (that case shipped in #1262), nested containers (#895), unions inside containers (#747), `type[...]` (no `Ty` variant; the D-109 16-byte ceiling is not the obstacle, as `frozenset[T]`'s measurement showed), the variadic `tuple[int, ...]` and empty `tuple[()]` (both rejected with `T0053`), and an empty `set()` literal (#927, still `C0001` at lowering). Inferring an empty `[]`/`{}`'s element type — from its annotation, from another binding of the same name anywhere in the function, or from a later `append`/`d[k] = v` — ships since #1021 (D-245); a position with no such evidence reports `T0003` | typing | `pep_0585_builtin_generics.py`, `dict_order.py`, `pep_0585_set_int.py`, `tuple_heterogeneous.py` | ◐ |
...
| PEP | Feature | Cat | Test | St |
|---|---|---|---|---|
| [604](https://peps.python.org/pep-0604/) | Union syntax, e.g. `int` or `str`. `Optional[T]` for `T` in `{int, float, bool}` type-checks and codegens end-to-end -- declared, assigned, returned, module-global and function-local (D-197/D-204, Parts 1 and 3 of [#747](https://github.com/rotnov/pycc/issues/747)); flow-sensitive narrowing back to plain `T` inside a top-level `is`/`is not None` branch, including the early-return continuation shape (D-205, [#769](https://github.com/rotnov/pycc/issues/769), Part 2 of #747). Not proven: a general union with neither side `None` or a 3+-operand chain (`T0048`); `Optional[T]` outside `{int, float, bool}` (`T0049`); narrowing through a compound `and`/`or` test or narrowing a name to `None` | typing | `pep_0604_union.py` | ◐ |
