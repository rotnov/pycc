...
| PEP | Feature | Cat | Test | St |
|---|---|---|---|---|
| [604](https://peps.python.org/pep-0604/) | Union syntax, e.g. `int` or `str`. `Optional[T]` for `T` in `{int, float, bool}` type-checks, codegens, and narrows via `is`/`is not None` (D-197/D-201/D-204, Parts 1-3 of [#747](https://github.com/rotnov/pycc/issues/747)). Not proven: a general union with neither side `None` or 3+ operands (`T0048`); `Optional[T]` outside `{int, float, bool}` (`T0049`); narrowing through `and`/`or` or to `None` | typing | `pep_0604_union.py` | ◐ |
