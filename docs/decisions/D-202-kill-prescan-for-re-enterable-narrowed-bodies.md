...
## D-202: Kill-prescan for re-enterable Optional[T] narrowed bodies (D-068 re-review of #780, third round)
- Status: accepted
- Context: D-201 (Part 2 of #747) implemented `Optional[T]` flow-sensitive
  ...
  step itself (see D-201's own Alternatives section and
...
- Decision:
  1. **Kill-prescan, applied uniformly before checking or lowering any
     ...
     this repository's existing D-127 judgment call (recorded in D-201's
...
- Consequences:
  - **Narrows D-201's Consequences claim** that "a narrowing fact ...
    never survives a reassignment of the narrowed name" — that statement
    was true only for the constructs D-201 itself covered (straight-line
  ...
  - `docs/decisions/D-201-optional-t-flow-sensitive-narrowing-part2.md`
    ...
    it instead) — a reader relying on D-201 alone should cross-reference
  ...
  - The kill-prescan is deliberately whole-body and non-positional: a read
    ...
    additional analysis passes — consistent with D-201's own
