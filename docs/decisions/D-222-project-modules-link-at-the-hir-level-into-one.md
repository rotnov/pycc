...
- Amendment (2026-09-24): #1280 lowers a multi-name `import a, b` instead of refusing it. Two statements above are superseded for `Stmt::Import`. First, the request/answer key: `pycc_hir::project_import_requests` asks one request per alias that has no `asname` and that `pycc_std` does not resolve, keyed by *that alias's own span*, so `ResolvedImports` is keyed by the alias span for a plain `import` and by the statement span for a `from ... import` (the "keyed by the import statement's span" clause under "The driver owns the filesystem" holds only for the latter). Second, the success condition: a `Stmt::Import` lowers alias by alias in source order, as CPython runs `import a` then `import b`, and fails as a whole on its first failing alias, with that alias's one diagnostic reported at the whole statement's span. It poisons nothing exactly when every alias resolves through `pycc_std`, and otherwise poisons every name it would have bound, as before. The "one alias, no `asname`" wording of the success condition above was already stale for the `asname` half since D-231's `import math as m` (Part 1 of #883). The statement *lowers* under a looser condition: every alias resolves through `pycc_std` or is answered foreign (Part 1 of #1026). A statement holding a foreign alias, such as `import sys, re`, therefore lowers and still poisons every name it binds -- the same deliberate divergence a single `import numpy` already had. A single-alias statement's diagnostics and spans are byte-for-byte unchanged.
- Alternatives:
