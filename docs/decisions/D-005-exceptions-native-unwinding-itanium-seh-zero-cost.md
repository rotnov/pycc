...
**Superseded by [D-173](./D-173-exception-propagation-via-global-state-superseding-d-005.md)** — v0.3 ships exception propagation via explicit per-thread runtime state instead of native unwinding. D-005's native-unwinding approach remains a viable future optimization once the exception model is stable and profiling justifies the cross-platform unwinding infrastructure cost.
