...
- Amendment (2026-09-24, [#1291](https://github.com/rotnov/pycc/issues/1291), Part 1 of #1282): point 6's "an alias on a project or unregistered module (#964) stay `C0001`" now holds only for a project module, a dotted module, and a module the driver does not answer. An undotted module that is neither a project module nor a `pycc_std` registration, and that the driver answers as CPython-backed, is a foreign import under D-244 rule 3, and `import X as Y` binds `Y` to the CPython module object. An alias that pycc would read by its spelling (any Python builtin such as `range`, a `pycc_std`-resolvable name such as `typing`, or a marker pycc recognises without an import: `TYPE_CHECKING`, the base-class markers `Enum`/`StrEnum`/`Protocol`/`ABC`, the decorator and dataclass markers, and the import-free annotation names) is refused with its own `C0001`, because the builtin lookups, the stdlib receiver fallback, the `TYPE_CHECKING` fold and the bare-name markers would otherwise give it a different meaning; `crates/pycc_hir/src/import/spelling.rs` holds the canonical list. D-222's poisonable-names clause is unchanged in kind: a foreign `import X as Y` lowers and still poisons `Y`.
