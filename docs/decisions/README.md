...
decision requires a new entry, not an edit -- see `TEMPLATE.md`. An accepted
or superseded entry's original lines are immutable: amendments are inserted
dated lines, only the two status lines may be replaced, and CI's `governance`
job enforces this with `scripts/check_decision_immutability.py` (D-240).
