...
because the ledger uses a single-backtick code span to project it exactly. An
existing active or diagnostic query may make only one lifecycle transition: to
`retired`, with a non-null retirement timestamp strictly after its final
measurement. Every identity, intent, KPI, alias, rationale, activation, and
