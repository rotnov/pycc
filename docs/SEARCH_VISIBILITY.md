...
## External-mention evidence and launch readiness gate (#209)

This section records the structured external-mention evidence model and
the non-spam launch readiness gate. The structured evidence lives in
`site/search-visibility/evidence.json` and is validated by
`scripts/check_search_visibility.rb`.

### Evidence model

Every external mention observation is recorded as an append-only entry.
Entries are never deleted or rewritten; later observations add new
entries that may supersede earlier ones.

Each entry records:

- `observed_at` — UTC timestamp of the observation.
- `discovery_surface` — where the observation was made.
...
- `sample_limitation` — known limitations of the observation.
- `source_limitation` — provider-stated limitations.
