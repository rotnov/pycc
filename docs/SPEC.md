...
| [conformance-breadth-manifest.json](../tests/fixtures/conformance-breadth-manifest.json) | Per-row breadth declaration for every evidence-backed (`◐`/`✅`) PYTHON_STANDARDS.md row: what its fixtures prove, and what the PEP contains that they do not, each gap classified `core` or `out-of-scope` (D-176, D-177). Its checker also binds `ROADMAP.md`'s stated conformance totals to the matrix (#623) | `scripts/check_conformance_breadth.py` |
...
| [TESTING.md](./TESTING.md) | 10 test layers, conformance harness, differential fuzzing, both corpora, corpus bot | CI, `pycc_testkit` |
...
| [sessions/](./sessions/README.md) | Informational handoff snapshots, at most one dated file per merged pull request (D-192): overall delivery status, in-flight work, resume points (not normative requirements) | session continuity/handoff only |
...
| `site/evidence-heroes.json` | Versioned evidence-hero inventory, ordered immutable execution transcripts, the commit-bound status snapshot, current scope, unavailable states, and cross-surface projections (D-186, D-230, D-241) | `scripts/check-site.sh`, `scripts/test-check-site.sh`, `scripts/check_status_snapshot.py`, `scripts/collect_status_snapshot.py`, HTML/Markdown/LLM/structured/social projections |
