...
| [DECISIONS.md](./DECISIONS.md) | ADR log D-001…D-056, D-062, and D-066…D-068 (int repr, LLVM, UTF-8 str, no-GIL model, coverage, agent surfaces, CI trust, iEvo, repository governance, Windows CI, roadmap evidence, frontend depth, performance-gate activation and audited recovery, paired-runner, source-aware and fixed-replicate stabilization, type-check environment sharing, local-binding classification, agent retrospective/session logs, source-installed pre-commit distribution, and pinned local review; open PR #132 has reconciled its former D-062/D-066 collisions and currently carries D-057…D-061 plus D-070…D-073) | irreversible calls |
