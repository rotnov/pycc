...
| [DECISIONS.md](./DECISIONS.md) | ADR log D-001…D-053 (int repr, LLVM, UTF-8 str, no-GIL model, coverage, agent surfaces, CI trust, iEvo, repository governance, Windows CI, roadmap evidence, frontend depth, tagged-fixnum `int` layout, corrected `str`-refcounting accepted-leak scope…) | irreversible calls |
