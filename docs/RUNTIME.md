...
bigint-valued elements/values (D-180 deliberately does not widen that
boundary: a container-held bigint reference has no owner today, and whoever
widens it must add an egress retain at every container read site first);
