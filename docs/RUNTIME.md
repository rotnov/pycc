...
- `str`: immutable UTF-8, `{len, hash-cache}` + bytes; small-string optimization ≤ 22 bytes inline. Codepoint indexing via lazily built offset index (amortized O(1), see D-007). **Current state (through PR-5):** every `str` value is a pointer to a refcounted heap object (small-string bytes inline in that same allocation, per D-059); reassigning a named `str` local, and top-level program completion, both decref the previous value. `pycc_codegen` inserts `incref`/`decref` calls only at codegen-time binding sites it can see in one static pass over the MIR, never via real control-flow- or lifetime-aware liveness tracking, which only `pycc_own` (v0.5) can provide -- three distinct, memory-safe (no double-free, no use-after-free), and potentially loop-unbounded leak classes follow from that scope (D-061's Task 7 addendum, correcting D-060's own narrower original wording -- see D-070): **(a) an unbound temporary** -- a `str` value produced but never bound to any name, e.g. a string literal or a comparison/concatenation/non-duplicate-call result consumed directly as an `if`/`while` condition or as an operand -- is never decremented at all; **(b) a loop-body-first binding** -- a `str` local whose *first* assignment lexically occurs inside a `while`/`for` body is not recognized as "already bound" by codegen's single static visit to that assignment, so every loop iteration but the last leaks its predecessor's value; and **(c) a function-scoped `str` local or parameter never reassigned before its function returns** is never decrefed at any of that function's return sites. All three are accepted, documented leaks until `pycc_own` makes real liveness tracking possible.
