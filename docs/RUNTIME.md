...
| `memoryview` | carried; accepts an exact `memoryview` **only** — no `bytes`, `bytearray`, `array.array` or NumPy array, even though each exports a buffer. It must additionally be C-contiguous (CPython's own `BufferError` is propagated verbatim otherwise), one-dimensional, and of element format `'d'` (float64); each of the last two raises a `TypeError` naming what it saw. A NumPy array therefore reaches the admitted slot only as a view over it -- `memoryview(a.reshape(-1))` for a C-contiguous `float64` array, which is zero-copy and one-dimensional -- and never as the array object itself; the four refusals that boundary owns (a bare `ndarray`, a two-dimensional view, an element format other than `'d'`, and a strided view) are pinned end to end in `tests/issue_1114_numpy_oracle.rs` (Part 3 of #1027, [#1114](https://github.com/rotnov/pycc/issues/1114)). The wrapper owns the `Py_buffer` for the whole call and releases it on every exit, including the success path; compiled code receives a `{ ptr, len }` pair and never the buffer or the owning object. In a build without `--ext` a `memoryview` in a *signature* -- a parameter or the return type -- is refused as `I0405`. Only a signature admits the type at all: a `memoryview` *declaration* (`x: memoryview`) is a `C0001` capability gap in both modes, because nothing in #1027 produces a value to bind to the name. Exactly one *read* of a name bound to a `memoryview` is implemented (Part 2 of #1027, [#1113](https://github.com/rotnov/pycc/issues/1113)): `v[i]`, which loads one float64 element through `pycc_rt_buffer_f64_get`. That helper owns the bounds check -- it compares the index against the `len` the wrapper copied out of `Py_buffer.shape[0]` and raises `IndexError` for anything outside `0..len`, negative indices included, since a negative index is refused rather than wrapped (D-108) -- so compiled code never reads outside the view, and the index itself must be an `int` (`T0021` otherwise). Every *other* read is still that same `C0001` gap wherever it appears -- an alias, a `for`, a call of the name, `len(v)`, an element store `v[i] = x`, an attribute, or handing it to another function -- and that holds inside the body of an `--ext` export whose parameter the signature admits: the parameter position exists so the wrapper can unpack the view for the compiled boundary, not so compiled code can operate on it as a value. A `Protocol` method's signature splits the two positions: a `memoryview` parameter is still `I0405` in a native build (a class genuinely can satisfy it under `--ext`, where an exported function receives the view and passes it inward), while a `-> memoryview` member is refused at its declaration as `C0001` in *both* modes, because no class could implement it in either | **not carried**: `C0003` — the wrapper has released the buffer by the time it would have to hand one back. That refusal is the export walk's, so it names a *public* function; the same return type on a function that walk never visits — a private one, a method, a specialization — is refused under `--ext` as the `C0001` capability gap a declaration gets, since #1027 produces no `memoryview` value to return |
