...
- Heap objects: no project-wide generic header exists. **Current state (through PR-10, D-104):** each heap object type defines its own header inline instead of sharing one common layout — both `PyStrObj` and `PyIntListObj` (the only two heap objects shipped so far) use just `rc: Cell<u32>` plus their own payload fields, with no `type_id`/`flags` anywhere in the actual runtime. A shared/generic header with cycle-tracking, shareable, and has-finalizer flags remains a possible future design once a real second consumer of those flags exists, not something either shipped object implements today.
