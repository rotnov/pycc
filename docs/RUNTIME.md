...
bigint-valued elements/values; their lengths and positional indices remain raw
runtime counters.

## Exceptions

**Current state (PR-22 Part 1, #382, D-172):** exception handling uses a
global exception state + explicit check-and-branch mechanism (D-172,
superseding D-005's native unwinding proposal). The runtime maintains a
global `EXCEPTION_ACTIVE` flag and `EXCEPTION_VALUE` pointer
(`*mut PyExceptionObj`). `raise` sets this state; `try`/`except` generates
explicit checks after the try body and dispatches to matching handlers.
The plain `extern "C"` ABI and abort-on-unwind safety property are
preserved — no native unwinding is used.
...
Converted runtime failure paths: integer floor division by zero, integer
modulo by zero, float true division by zero, float floor division/modulo
by zero, list index out of range, and missing dictionary key now
set the exception state and return a neutral value instead of panicking.
Generated code checks for active exceptions after each top-level statement
and after each `try` body.

Uncaught exceptions at the top level are printed to stderr
(`ExceptionType: message`) and the process exits with code 1.

**Planned (post-Part 1):** `except*` groups (PEP 654), full traceback
with `.py` lines, implicit exception context (`__context__`), nested
try/except bare-raise stacking, exception checks after every
potentially-raising operation (not just top-level and try-body checks).
