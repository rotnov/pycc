...
- **Source.** The closure is read offline from the installed `*.dist-info`
  ...
  (#1290) when every import of it sits, at any depth, in the body of a
  `try` (or `try`/`except*`) at module level or nested only in
  module-level `if`/`try` blocks, with a bare `except:` or a handler
  naming `ImportError`, `ModuleNotFoundError` or `Exception`, alone or in
  a tuple; a handler, `else` or `finally` body is not guarded by its
