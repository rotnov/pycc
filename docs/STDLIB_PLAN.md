...
| `dataclasses`, `enum`, `typing`, `abc` | v0.3 (partial) | typing = compile-time only, zero runtime cost. **Shipped so far:** only the marker symbols the class model actually implements, registered in `crates/pycc_std/src/lib.rs` — `enum.Enum`, `typing.Protocol`, `typing.runtime_checkable`, `typing.override`, `typing.dataclass_transform`, `abc.ABC`, `abc.abstractmethod`, and `dataclasses.dataclass` (#579). Each is a compile-time marker with no runtime component; pycc also recognizes every one of them as a bare name without the import (the `Final`/`Annotated`/`Enum` precedent), so registering them buys import *resolution*, which is what CPython requires and therefore what the byte-for-byte conformance fixtures need. Everything else in these modules — `dataclasses.field`/`asdict`/`replace`, `enum.IntEnum`/`auto`, the rest of `typing` — stays unregistered and rejected with the ordinary `C0002` |
