...
- **New shared predicate** `pycc_hir::declares_name_outside_class_attrs` (`crates/pycc_hir/src/class/shadow.rs`, a new submodule rather than more lines in the 4.5k-line `class.rs`). It answers whether a class declares a name in `methods`, `static_methods`, `class_methods`, `properties` or `enum_members`. `attrs` (instance slots) and `protocol_members` are excluded on purpose — a class object has no instance `__dict__`, and a protocol member is an interface requirement rather than a binding. `abstract_methods` is redundant (pushed into `methods` at lowering) and `dataclass_fields` is a subset of `attrs`.
...
- The branch is committed and gate-green locally; the next step is the pinned local reviewer and then the PR.
...
- `wc -c docs/ROADMAP.md` is 168805 against `scripts/check-site.sh`'s 168960-byte ceiling — 155 bytes of headroom left. The next roadmap sentence will not fit; raising `budget_bytes` for that entry in `site/llms-txt-context-manifest.json` is the intended route (the aggregate ceiling has room), not trimming unrelated prose.
