...
| Public evidence and discoverability | Four-page publication package with measured discovery | [`site/`](../site/) contains the canonical landing page plus commit-relative status, implemented-versus-target architecture, and AI-native experiment pages. The website validator enforces unique metadata, connected structured data, internal navigation, visible authorship/status disclosures, the exact sitemap set, and the landing page's exact relative stylesheet plus deferred executable script references; negative asset-reference mutations also reject inert references, foreign-subtree asset tags, suppressing or execution-changing attributes, duplicate attributes, and base-URL overrides so required CSS or JavaScript cannot become browser-orphaned. The Pages workflow publishes the package from `main`; its best-effort IndexNow job batch-submits every canonical sitemap URL, and a hermetic HTTP fixture exercises the real notifier success and failure paths. [`SEARCH_VISIBILITY.md`](./SEARCH_VISIBILITY.md) preserves timestamped GitHub Search positions, rolling traffic snapshots, discovery-surface changes, and independent Search Console URL/sitemap/performance states without attributing automation-heavy activity to SEO. All four canonical website URLs are individually indexed in Google at this revision, while external query performance and sitemap processing remain monitored observations rather than repository capability claims. |
