...
| Quality gates | Frontend gates enforced | Unit, diagnostic-snapshot, and slice-level end-to-end tests pass; CI enforces 100% Rust line and region coverage and requires both untrusted `frontend-perf-measure` and isolated, hash-verified `frontend-perf-gate` through `ci-gate`, so a measured >2% frontend regression blocks merge without executing PR-head comparator code (D-044/D-046). The conformance harness, five-target language conformance, fuzzing, and corpus layers remain planned according to [TESTING.md](./TESTING.md). |
