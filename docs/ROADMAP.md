...
| Quality gates | Frontend and source-aware paired performance gates enforced | Unit, diagnostic-snapshot, and slice-level end-to-end tests pass; CI enforces 100% Rust line and region coverage through `ci-gate`. The check-only type path validates the original HIR against inferred signatures without constructing the resolved clone needed only by downstream compiler stages, and per-function environments share the immutable module function registry through copy-on-write storage while keeping local bindings isolated. `frontend-perf-measure` and the isolated `frontend-perf-gate` remain required through `ci-gate`. D-051/D-053 superseded D-048's cross-run artifact transport: the workflow measures the exact predecessor and candidate sequentially on one hosted runner, seals the predecessor timing before candidate code runs, binds both downloads to distinct numeric artifact IDs, and flattens each into its exact destination. D-056 adds trusted pre-execution identity for complete `src/`/`crates/` trees alongside the existing bound build inputs. Identical executable inputs keep the measured delta as visible, non-blocking environment telemetry; changed source retains D-044's hard `>2%` median block. The active workflow is byte-identical to the reviewed [D-056 source-aware fixture](../tests/fixtures/d56-source-aware-ci.yml), and only its digest is authorized. The D-051 fixture and comparator remain historical audit evidence, while D-048 is retired. Revision, benchmark-contract, artifact-identity, file-set, comparison, executable-input identity, and phase drift fail closed in focused tests. The conformance harness, five-target language conformance, fuzzing, and corpus layers remain planned according to [TESTING.md](./TESTING.md). |
