...
Last reviewed on 2026-07-26. This section describes the repository tree in the commit that contains it: behavior and evidence from that same commit count, while work that exists only in another open pull request or unmerged branch remains work in flight.
...
| Quality gates | Frontend and performance gates enforced | Unit, diagnostic-snapshot, and slice-level end-to-end tests pass; CI enforces 100% Rust line and region coverage through `ci-gate`. The check-only type path validates the original HIR against inferred signatures without constructing the resolved clone needed only by downstream compiler stages, and pre-sizes internal signature, function, and per-function binding tables from already-known HIR cardinalities to avoid allocator growth without changing lookup semantics. `frontend-perf-measure` and the isolated `frontend-perf-gate` are required through `ci-gate`, fail closed without exact-predecessor evidence, and block a measured regression over 2%. During the staged D-051 migration, the active workflow is selected by reviewed whole-file digest: the D-048 [steady-state fixture](../tests/fixtures/d48-steady-ci.yml) downloads the non-expired artifact from the exact successful `main` predecessor; the prospective D-051 [paired fixture](../tests/fixtures/d051-paired-ci.yml) instead benchmarks that exact predecessor, seals its immutable artifact ID plus archive/file digests before executing current source, and measures the current compiler crates through the predecessor-owned harness, manifest, lockfile, and Cargo configuration on the same runner. It downloads only the sealed ID and compares both same-run artifacts through the predecessor-owned hash-verified checker. The checker accepts either complete reviewed job pair but rejects mixed generations. Activation is a later workflow-only replacement; no bootstrap variable or missing-baseline exception is introduced. The conformance harness, five-target language conformance, fuzzing, and corpus layers remain planned according to [TESTING.md](./TESTING.md). |
