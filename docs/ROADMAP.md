...
| Quality gates | Source-aware gate active; changed-source fixed replicates staged | Unit, diagnostic-snapshot, and slice-level end-to-end tests pass; CI enforces 100% Rust line and region coverage through `ci-gate`. The check-only type path validates the original HIR against inferred signatures without constructing the resolved clone needed only by downstream compiler stages, and per-function environments share the immutable module function registry through copy-on-write storage while keeping local bindings isolated. `frontend-perf-measure` and the isolated `frontend-perf-gate` remain required through `ci-gate`. D-056 is active and byte-identical to the reviewed [source-aware fixture](../tests/fixtures/d56-source-aware-ci.yml): complete identical executable inputs make timing visible non-blocking telemetry, while changed inputs retain D-044's hard `>2%` block. Exact same-source-pair PR/main evidence reopened [#109](https://github.com/rotnov/pycc/issues/109) because changed-input single observations produced `+0.10%` and `+3.66%`. D-062 therefore stages, but does not activate, a byte-exact [source-aware five-replicate successor](../tests/fixtures/d62-replicated-paired-ci.yml): D-056's exact-identity path remains deterministic, while changed inputs use the median of five retained per-run medians. The allowlist contains only active D-056 and inert D-062. Revision, benchmark-contract, trust ordering, executable-input identity, artifact identity, exact ten-file evidence, and comparison drift fail closed; activation requires a fresh pull request after this staging state reaches `main`. The conformance harness, five-target language conformance, fuzzing, and corpus layers remain planned according to [TESTING.md](./TESTING.md). |
