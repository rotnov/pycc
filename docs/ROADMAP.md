...
| Quality gates | Frontend gates enforced; performance activation staged | Unit, diagnostic-snapshot, and slice-level end-to-end tests pass; CI enforces 100% Rust line and region coverage through `ci-gate`. D-048 supersedes D-047's temporary PR-6 deferral and replaces D-046's ref-scoped cache transport with exact-predecessor artifacts. The exact activation and bootstrap-free steady-state workflow digests plus their complete lifecycle validators are staged while the pre-split workflow remains active; D-049 also preserves that exact pre-split workflow as an inert predecessor fixture so transition tests do not depend on live workflow state. The immediately following exact-head activation and post-seed cleanup changes make the >2% frontend regression check merge-blocking. Until those two changes merge, the performance gate is not yet active. The conformance harness, five-target language conformance, fuzzing, and corpus layers remain planned according to [TESTING.md](./TESTING.md). |
