...
| Quality gates | Frontend and paired performance gates enforced; source-aware successor staged | Unit, diagnostic-snapshot, and slice-level end-to-end tests pass; CI enforces 100% Rust line and region coverage through `ci-gate`. The check-only type path validates the original HIR against inferred signatures without constructing the resolved clone needed only by downstream compiler stages, and per-function environments share the immutable module function registry through copy-on-write storage while keeping local bindings isolated. `frontend-perf-measure` and the isolated `frontend-perf-gate` remain required through `ci-gate` and retain D-044's greater-than-2% block. D-051/D-053 supersede D-048's cross-run artifact transport: the active workflow measures the exact predecessor and candidate sequentially on one hosted runner, seals the predecessor timing before candidate code runs, binds both downloads to distinct numeric artifact IDs, flattens each into its exact destination, and compares their medians through the hash-verified predecessor-owned checker. The active workflow is byte-identical to the reviewed [D-051 paired fixture](../tests/fixtures/d51-paired-ci.yml); only its digest is active, while the D-048 digest and fixture are retired. D-056's reviewed [source-aware fixture](../tests/fixtures/d56-source-aware-ci.yml) and distinct hash-verified comparator are staged but not active: after a separate fresh-base activation, identical `src/`/`crates/` plus the existing bound build inputs will make timing non-blocking environment telemetry, while any executable-input change keeps the same `>2%` block. Revision, benchmark-contract, artifact-identity, file-set, comparison, executable-input identity, and phase drift fail closed in focused tests. The conformance harness, five-target language conformance, fuzzing, and corpus layers remain planned according to [TESTING.md](./TESTING.md). |
