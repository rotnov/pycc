...
| Agent tooling | Codex/Claude parity enforced | Repository-owned instructions and skills target both Codex and Claude Code. The `agent-assets` gate validates skill parity and the immutable iEvo baseline, and rejects required tracked instructions, overlays, tests, workflows, helpers, source files, or executables that depend on optional unpinned Claude plugins. Workflow helper discovery applies effective workflow/job/step working directories recursively, while optional GitHub marketplace coordinates follow GitHub's case-insensitive owner/repository identity. |
