...
| Distribution | Source hook available; release evidence incomplete | The main repository publishes `pycc-check` through [`.pre-commit-hooks.yaml`](../.pre-commit-hooks.yaml). It passes all selected Python paths to one frontend-only process and has clean exact-revision installation evidence on the primary macOS arm64 host. The current `language: rust` package graph still requires Rust 1.97.1 and LLVM 22.1.1, and Tier-1 installation evidence plus a release tag remain open under [DISTRIBUTION.md](./DISTRIBUTION.md). |
