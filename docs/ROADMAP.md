...
| Repository governance | Enforced | Protected `main` requires an up-to-date PR, `build-test-coverage`, the trusted `audit` context, and resolved conversations. The required build runs clean-clone agent-policy validation, while the push-time [main-history audit](../.github/workflows/main-history-audit.yml) uses an immutable checker and correlates each introduced commit with the merged-main PR whose merge commit arrived in that push; a missing or failed run is release-blocking under [REPOSITORY_GOVERNANCE.md](./REPOSITORY_GOVERNANCE.md). |
