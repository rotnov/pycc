...
| Language surface | v0.1 frontend and codegen both implemented, with documented gaps | `pycc check` accepts and type-checks the v0.1 grammar covered by this row (`build`/`run` implement the same coverage) -- arithmetic (including add/sub and smallint-product overflow-to-bigint per D-001/D-058/D-061, true-division-always-`float` promotion with explicit zero-divisor failure, and CPython-compatible float floor division/modulo), comparisons (`int`/`float`/`bool`/`str`, lexicographic for `str`), `if`/`while`/`for`+`range`, functions/recursion, `str` (small-string optimization per D-007/D-059, concatenation, refcounted for named locals), basic f-strings, and `print` for every v0.1 scalar type. Known, accepted gaps (see D-061/D-074 and Task 7/9's scope notes in [`docs/superpowers/plans/2026-07-25-pr5-codegen-depth.md`](./superpowers/plans/2026-07-25-pr5-codegen-depth.md)): comparing an already-promoted bigint `int`, `int` multiplication/floor-division/modulo/power on a bigint operand (including converting an already-promoted bigint to `float`, and floor-dividing the minimum taggable `int` by `-1`), `float ** float` where the base is zero raised to a negative power, a negative base raised to a non-integer power (would need a complex result `pycc` doesn't support), or a finite operand pair whose true result overflows `float` range, `float` formatting outside CPython's own non-scientific-notation range, unbound `str` temporaries, and `str` parameters/locals at function return are honest unsupported boundaries or documented memory-safe leaks, not silent wrong answers. One gap is a genuine silent deviation rather than a panic or a leak: once a `bool` value crosses an `int`-typed assignment/parameter/return/`range` boundary (D-061/D-074's tagged-int widening), its runtime representation becomes indistinguishable from an ordinary `int`, so `print`/`str` on that value renders CPython's `"True"`/`"False"` as `"1"`/`"0"` instead -- v0.1's unboxed scalar representation has no spare bit to record that a given tagged int originated as a `bool`. Closing this needs a representation change (e.g. a runtime type tag) beyond this PR's scope, so it is tracked here rather than fixed. Valid Python outside the implemented HIR subset, including unary operators (`-x`/`+x`/`not x`/`~x`), produces a spanned `C0001` capability diagnostic before MIR/backend lowering and does not abort a multi-file `check` batch. |
