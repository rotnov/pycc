...
| Language surface | v0.1 frontend implemented; codegen pending | `pycc check` accepts and type-checks arithmetic, comparisons, assignments, function arguments/returns and recursion, `if`/`while`/`for`+`range`, booleans/floats/strings, and basic f-strings. Valid Python outside that implemented HIR subset produces a spanned `C0001` capability diagnostic and does not abort a multi-file check batch. `build`/`run` remain intentionally limited to module-level `print(<i64 literal>)`, zero-argument function definitions, and explicit zero-argument calls until PR-5; frontend-accepted but unsupported backend constructs reach the explicit PR-5 boundary in [`pycc_mir`](../crates/pycc_mir/src/lib.rs). |
