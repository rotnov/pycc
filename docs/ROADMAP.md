...
| Repository governance | Enforced | Protected `main` requires an up-to-date PR, `build-test-coverage`, the trusted `audit` context, and resolved conversations. The required build runs clean-clone agent-policy validation, while the push-time [main-history audit](../.github/workflows/main-history-audit.yml) verifies that every resulting commit belongs to a merged PR targeting `main`; failures are release-blocking under [REPOSITORY_GOVERNANCE.md](./REPOSITORY_GOVERNANCE.md). |
