...
| CLI | Partial | `build`, `run`, `version`, and explicit-file `check` have working slice-level paths. `check` accepts multiple native paths, aggregates failures, and stops before code generation; directory/project discovery, `test`, `explain`, `init`, `clean`, and the broader flag contracts in [CLI_SPEC.md](./CLI_SPEC.md) remain planned. |
| Diagnostics | Partial | Parser failures become `L0001` and unsupported valid programs in the implemented HIR boundary become `C0001`, both with source spans and terminal-safe human rendering. Python source encoding declarations, mixed-file exit precedence, Unicode caret alignment, and native path handling are covered. The full shipped-feature registry, screenshot parity, JSON output, and broader diagnostic catalog remain open. |
