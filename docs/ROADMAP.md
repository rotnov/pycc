...
| Quality gates | Frontend and performance gates enforced; corrected paired successor staged | Unit, diagnostic-snapshot, and slice-level end-to-end tests pass; CI enforces 100% Rust line and region coverage through `ci-gate`. The check-only type path validates the original HIR against inferred signatures without constructing the resolved clone needed only by downstream compiler stages, and per-function environments share the immutable module function registry through copy-on-write storage while keeping local bindings isolated. D-048 supersedes D-047's temporary PR-6 deferral and replaces D-046's ref-scoped cache transport with exact-predecessor artifacts. `frontend-perf-measure` and the isolated `frontend-perf-gate` are required through `ci-gate`; the bootstrap-free gate downloads the non-expired `frontend-perf-current` artifact from the exact successful `main` predecessor, fails closed when that baseline is unavailable, and blocks a measured regression over 2%. The one-time activation variable, bootstrap branches, and earlier transition fixtures are removed; the reviewed [D-048 steady-state fixture](../tests/fixtures/d48-steady-ci.yml) remains byte-identical to the active workflow. D-051 additionally stages an inert [paired-runner successor](../tests/fixtures/d51-paired-ci.yml) after three docs-only attempts exposed cross-host timing offsets. Its original prospective digest was retired when activation exposed nested exact-ID downloads; the corrected candidate explicitly flattens both artifacts and preserves the 2% threshold, but is not active until a separate byte-exact workflow pull request lands. The conformance harness, five-target language conformance, fuzzing, and corpus layers remain planned according to [TESTING.md](./TESTING.md). |
