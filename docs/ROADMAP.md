...
| Quality gates | Frontend and performance gates enforced | Unit, diagnostic-snapshot, and slice-level end-to-end tests pass; CI enforces 100% Rust line and region coverage through `ci-gate`. D-048 supersedes D-047's temporary PR-6 deferral and replaces D-046's ref-scoped cache transport with exact-predecessor artifacts. D-051 completes that transition: the tracked [CI workflow](../.github/workflows/ci.yml) is byte-identical to the reviewed [steady-state fixture](../tests/fixtures/d48-steady-ci.yml), `frontend-perf-measure` and the isolated `frontend-perf-gate` are required through `ci-gate`, and every comparison requires the non-expired artifact from the exact successful `main` predecessor with no missing-baseline bootstrap. The conformance harness, five-target language conformance, fuzzing, and corpus layers remain planned according to [TESTING.md](./TESTING.md). |
