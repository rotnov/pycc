...
| Distribution | Source hook available; release evidence incomplete | The main repository publishes `pycc-check` through [`.pre-commit-hooks.yaml`](../.pre-commit-hooks.yaml). It passes selected Python paths to serial frontend-only batches with at most one hook process running at a time, and has clean exact-revision installation evidence on the primary macOS arm64 host. The current `language: rust` package graph still requires Rust 1.97.1 and LLVM 22.1.1, and Tier-1 installation evidence plus a release tag remain open under [DISTRIBUTION.md](./DISTRIBUTION.md). The `workflow_dispatch`-only [`hook-install-check.yml`](../.github/workflows/hook-install-check.yml) now exists to gather that evidence on all five Tier-1 targets; it has not yet been dispatched against a candidate revision, so this row stays open until it has and the per-target outcome is recorded in DISTRIBUTION.md. |
