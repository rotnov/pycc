...
| Agent tooling | Project-local alpha | The reviewed `i-have-an-issue` research workflow and the alpha `pycc` / consent-gated `pycc-feedback` skills are discoverable in both Codex and Claude Code. Automated validation covers immutable provenance, both entrypoint wiring paths, structural eval manifests, bounded compiler fixtures, and hash-bound evidence from the required manual behavioral runs; it does not claim to grade model semantics in CI. The public skills.sh page remains blocked on its stale index; [`vercel-labs/skills#1776`](https://github.com/vercel-labs/skills/issues/1776) tracks the external rescan. |
