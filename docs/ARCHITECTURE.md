...
- Pure/native-only artifacts use static linking by default on Linux (musl optional), a self-contained `.exe` on Windows, and a notarization-friendly binary on macOS. Planned embedded interop artifacts (D-128) instead form a self-contained application bundle containing their pinned CPython/package/native-library closure; they must not fall back to a target machine's Python installation; an embedded executable links libpython from the bundle's shared library by default, or statically from the interpreter's archive with its C-API symbols exported under `--static-libpython` ([D-251](decisions/D-251-static-libpython-link-for-embedded-executables.md)); on a Windows host it is instead a stub `OUT` importing only the system DLLs `KERNEL32` and `ntdll` that loads a program DLL, linked against `python314.dll`, from `OUT.pycc\` ([D-253](decisions/D-253-windows-embedded-executable-a-stub-out-loading-a.md)). The hosted `ext` artifact is the deliberate exception: it is a CPython extension module loaded by, and resolving its C-API symbols from, a host interpreter admitted by D-244 rule 1 — a GIL-enabled build at or above the artifact’s stable-ABI floor, never a free-threaded one. Its link line is the host toolchain driver again, with the platform's shared-object arm added: `-bundle -undefined dynamic_lookup` on macOS, `-shared` on Linux, and `-shared` plus `-lpython3` against the interpreter's `libs` directory on Windows, selected from the `--target` triple rather than from the building host ([#1036](https://github.com/rotnov/pycc/issues/1036)).
