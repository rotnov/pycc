...
| `pycc_scratch` | `ScratchDir`: shared RAII scratch-directory abstraction with panic-safe `Drop` cleanup and collision-safe naming, used by tests/benchmarks/CLI helpers instead of ad hoc `std::env::temp_dir().join(...)` calls (issue #779/#781, D-200) |
