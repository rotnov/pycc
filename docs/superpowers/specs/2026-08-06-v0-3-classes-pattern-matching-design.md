...
| 681 | `dataclass_transform` | no fixture — permanently `☐` per [D-196](../../decisions/D-196-pep-681-is-a-second-itemized-row-permanently.md) ([#749](https://github.com/rotnov/pycc/issues/749)): pycc treats `@dataclass_transform()` as `@dataclass` while CPython synthesizes nothing, so no oracle fixture can pass by construction | PR-18 (row only; the `@dataclass_transform` acceptance itself still lands here) | Explicitly named alongside 557 in TYPE_SYSTEM.md's Class model section; the fixture originally assigned here was never registered and was deleted in #749 |
