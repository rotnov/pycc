...
- **Gate:** ratio ≥ 20. This is PR-8's own benchmark harness to build (new script, e.g. `scripts/check_nbody_speedup.rb` alongside the existing `check_frontend_throughput.rb`, or a `tests/nbody_bench.rs` — PR-8's implementer picks the shape and records it as an ADR, the same way D-079 picked `tests/conformance.rs`'s shape for v0.1). (D-095/D-096 later documented per-target exceptions to this 20x figure — 12x on macOS aarch64, 15x on `windows-latest` — backed by real measurements; see `docs/DECISIONS.md` and `docs/ROADMAP.md`'s Quality-gates row for the current, authoritative per-target values.)
