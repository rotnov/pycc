...
- **Method:** same-machine paired comparison (not the cross-runner D-051/D-053/D-056/D-062 regression-gate machinery — this compares two different programs on one machine at one time, not one program's speed across two commits). Both programs run `K = 5` times; take the wall-clock and exact-child CPU-time median of each. D-126 Phase A keeps the gate on median(CPython wall-clock) ÷ median(pycc `--release` binary wall-clock), while also recording median(CPython CPU time) ÷ median(pycc `--release` binary CPU time) and all four medians on every run. A later Phase B may switch the gate only after at least five observations exist for every Tier-1 leg and its CPU-time floors are derived from those observations rather than copied from the wall-clock contract.
