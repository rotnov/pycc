...
- **Method:** same-machine paired comparison (not the cross-runner D-051/D-053/D-056/D-062 regression-gate machinery — this compares two different programs on one machine at one time, not one program's speed across two commits). Both programs run `K = 5` times; take the wall-clock and exact-child CPU-time median of each. The gate is median(CPython wall-clock) ÷ median(pycc `--release` binary wall-clock), while median(CPython CPU time) ÷ median(pycc `--release` binary CPU time) and all four medians are also recorded on every run. D-129 evaluated D-126's five observations per Tier-1 leg and retained this wall-clock gate because CPU-time variance was effectively unchanged on Ubuntu x86_64 and materially worse on Windows; CPU time remains non-gating diagnostic telemetry.
- **Profile:** pycc side built `--release` (LTO on); CPython side is the pinned oracle (`python3.14` → 3.14.6) with no special flags.
- **Gate:** ratio ≥ 20. This is PR-8's own benchmark harness to build (new script, e.g. `scripts/check_nbody_speedup.rb` alongside the existing `check_frontend_throughput.rb`, or a `tests/nbody_bench.rs` — PR-8's implementer picks the shape and records it as an ADR, the same way D-079 picked `tests/conformance.rs`'s shape for v0.1). (D-095/D-096/D-101 later documented per-target exceptions to this 20x figure — 12x on macOS aarch64, 15x on `windows-latest`, 18x on `ubuntu-24.04-arm` — backed by real measurements; see `docs/DECISIONS.md` and `docs/ROADMAP.md`'s Quality-gates row for the current, authoritative per-target values.)
...
| 649/749 | Deferred annotations | `py314/pep_0649_deferred_ann.py` | deferred past v0.2 | **Not reachable at v0.2, wrong like the original 526/594 rows** — empirically verified (PR-9 planning, 2026-07-30): CPython 3.14's actual observable PEP 649 behavior is that an annotation expression referencing an undefined name is never evaluated unless `__annotations__` is accessed (`def f(x: DoesNotExistAnywhere): ...` then calling `f` prints fine, confirmed against real CPython 3.14.6). pycc's static, eager Ty-resolution model rejects that exact program today with `C0001` (`type annotation ... is not supported yet`) — by design, since every annotation must resolve to a concrete `Ty` at check time; a fixture that actually exercises deferred evaluation is therefore fundamentally incompatible with pycc's current architecture, not merely unimplemented. The closest realistic case where deferred evaluation becomes observably necessary is a self-referential forward reference inside a class body (`class Node: def next(self) -> Node: ...`), which needs a class model that doesn't exist before v0.3 (confirmed: `class` statements produce `C0001` today, unconditionally). Reassign once v0.3 gives pycc some notion of a not-yet-resolvable annotation target |
...
**Update (PR-9 planning, 2026-07-30):** PEP 649/749's row above is corrected — it is not reachable at v0.2, the same class of wrong "already true" claim this section's own intro already caught once for PEP 526/594. That drops the v0.2-reachable pool from 16 to **15 fixtures against the ≥15 target — zero margin remains**, not one item. PR-9 itself is unaffected in row count (it never depended on 649/749 completing; see below), but every one of PR-10 through PR-14's own remaining rows must now land for the milestone's conformance bullet to be satisfiable at all; a single further unfixturable row (matching what already happened twice for 526/594 and once more here for 649/749) would make v0.2's own acceptance criterion unreachable as currently scoped, and should be escalated immediately rather than discovered at PR-14.

16 fixtures were originally counted against the ≥15 target, one item of margin (see the Update above — this is now 15 against 15, zero margin). PR-9 owns 8 rows that need no new language feature (proving what v0.1 already does — one fewer than originally counted, since 649/749 is no longer one of them) plus PEP 526's own small bounded addition — it is not purely harness-only, but its new scope is narrow and well-defined, which is why it's still sequenced right after the perf harness and before any container work.
