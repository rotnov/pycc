...
## Correction #5 (2026-08-22, recorded while retiring D-103's residue under issue #275): the D-103 manifest mechanism this plan is built around no longer exists

Read this before acting on anything below. D-103's policy-successor-manifest stage-then-activate mechanism was retired by [D-172](../../decisions/D-172-nonblocking-property-based-ci-policy-audit.md) in PR #570 (merged 2026-08-17), which removed `validate_policy_successor_transition` from `scripts/check_ci_permissions.rb`. That checker no longer reads `tests/fixtures/policy-successor-manifest.json` at all, and no manifest entry can force a later activation. `.github/workflows/workflow-policy.yml` still reads the manifest, but only as a bounded inventory of paths the `audit` job materializes as data.

Consequently, every passage below describing that check as an active constraint is historical record of a real 2026-08-01 investigation, not current policy. Corrections #1 through #4 are preserved verbatim for that reason. Two specific claims are now void:

- Correction #4's point 1 concludes that Correction #3's "breaks the whole repo while staged" finding is "still live and growing more urgent by the day." It is not: the mechanism that produced that behavior is gone.
- The Global Constraints bullet beginning **D-103 / policy-successor-manifest — mandatory local pre-push gate.** is obsolete. There is no longer any reason to reproduce the `workflow-policy.yml` audit locally before pushing a change to a manifest-listed file; the check it was guarding against cannot fire. The one live constraint that remains is that `workflow-policy.yml` throws when a manifest-listed path is absent from the head tree, so a change that renames or deletes such a path must update the manifest in the same pull request.
...
## Correction #4 (2026-08-01, found empirically while executing round 7): activating `ci.yml` alone breaks `ci-gate`, and self-authorization forbids fixing it in the same commit — this needs an administrator-authorized emergency bypass, not another silent round

Round 6 (propose-ci.yml, PR #277) staged only `.github/workflows/ci.yml`'s new content. It did **not** also stage a fix for `scripts/test_check_roadmap_evidence.rb`'s two assertions that hard-code the live workflow's digest as `D100_COMPOSE_D91_D99_CI_WORKFLOW_SHA256` (`test_tier1_workflow_authorization_is_the_active_d100_digest` at `scripts/test_check_roadmap_evidence.rb:981`, and `test_d100_composed_workflow_is_active_and_reviewed` at `:1453`). That omission produces a genuine one-round deadlock for round 7, confirmed three separate ways:

1. **`ci.yml` activation is now mandatory in *every* future round, not just round 7.** Empirically verified: a synthetic "unrelated" candidate that leaves `.github/workflows/ci.yml` untouched (still matching `main`'s current, un-activated D100 content) fails `validate_policy_successor_transition` with `candidate protected policy target .github/workflows/ci.yml lacks a base-staged successor` — because `main`'s own manifest already has this entry staged (`source_path` → `tests/fixtures/policy-successors/ci.yml`, holding D112) since round 6 merged. The check resolves `expected` from `main`'s manifest and `main`'s own staged file, never from the candidate's manifest, so a candidate cannot opt out by editing its own copy of the pointer. This confirms Correction #3's "breaks the whole repo while staged" finding is still live and growing more urgent by the day.
2. **Self-authorization independently forbids fixing the two assertions in the same commit that activates `ci.yml`.** `scripts/test_check_roadmap_evidence.rb` is itself a protected target (steady state, `source_path == path`, in `main`'s manifest). Any edit to its real bytes without a prior, separately-merged staged successor fails `validate_policy_successor_transition` with `candidate protected policy target scripts/test_check_roadmap_evidence.rb lacks a base-staged successor` — reproduced directly against the harness (this round's own first attempt).
3. **Both constraints together are unsatisfiable in one round.** `ci-gate` (required; confirmed via `gh api repos/rotnov/pycc/branches/main/protection/required_status_checks` → `["audit","ci-gate"]`) depends on `build-test-coverage` (`.github/workflows/ci.yml:817-825`), which runs `ruby scripts/test_check_roadmap_evidence.rb` against the **candidate's own** checked-out files (not a trusted-base substitution — that substitution is `audit`'s job alone). Verified directly: a candidate with `ci.yml` activated to D112 and `scripts/test_check_roadmap_evidence.rb` left untouched produces exactly two failures and nothing else:
   ```
   1) Failure:
   RoadmapEvidenceCliTest#test_tier1_workflow_authorization_is_the_active_d100_digest [scripts/test_check_roadmap_evidence.rb:981]
   2) Failure:
   RoadmapEvidenceCliTest#test_d100_composed_workflow_is_active_and_reviewed [scripts/test_check_roadmap_evidence.rb:1453]
   135 runs, 550 assertions, 2 failures, 0 errors, 0 skips
   ```
   So: activating `ci.yml` alone is audit-green but `ci-gate`-red; fixing the test file in the same commit is `ci-gate`-green but audit-red; leaving `ci.yml` un-activated is audit-red for every PR in the repository, including unrelated ones. No single-commit, self-authorized round satisfies all three required-check constraints at once.

**Resolution (minimum-cost, not three more silent rounds):** the two-round shape that actually works is:
- **Round 7a**: activate `ci.yml` for real (matches its already-`main`-staged successor, self-authorized) **and**, in the same commit, stage the fixed test content at `tests/fixtures/policy-successors/test_check_roadmap_evidence.rb` with that file's manifest entry repointed there — leaving the *real* `scripts/test_check_roadmap_evidence.rb` bytes untouched. This combination passes `audit` (verified: staging a successor without touching real bytes never trips self-authorization). It leaves exactly the two known `ci-gate` failures above, and only those — this needs a deliberate, disclosed **emergency exception** to merge red, not a silent bypass.
- **Round 7b**: activate the test file (real bytes = the now-`main`-staged successor from 7a, manifest entry back to steady). This round is a normal, fully green merge on both required checks — `audit` passes because the successor is now base-staged, and `ci-gate` passes because the live assertions now match the live (D112) `ci.yml`.

**This is explicitly out of scope for autopilot to execute unilaterally.** `docs/REPOSITORY_GOVERNANCE.md`'s Emergency Path is unambiguous: "Only a repository administrator may temporarily edit protection... An agent, bot, or normal maintenance task never receives standing bypass permission." The user's earlier "сам решай, ты же в автопилоте" delegation covered *which technical remedy* to pursue for issue #109 (macOS→Linux runner move) — a technical choice with no shared-state blast radius. Merging round 7a past a required, green-by-design `ci-gate` is a different category of action entirely: it is exactly the scenario D-054/incident #125 both document and gate behind explicit, disclosed, time-boxed administrator authorization. This plan does not proceed past this point without that authorization.

**Standing fact regardless of the above**: `main` is *already* failing `audit` for every open and future PR in the repository right now, and stays that way until round 7a merges. This is the reason to resolve this quickly, not a reason to act unilaterally.

**Follow-up, tracked separately, not folded into this remedy**: the root cause was that round 6 staged `ci.yml` without also co-staging every protected target whose content must change in lockstep with it. Record a rule — "a D-103 propose round must co-stage every protected target coupled to the change, not just the one the round is nominally about" — in `AGENTS.md` or a new ADR once this deadlock clears, so a future propose round doesn't reproduce it.
