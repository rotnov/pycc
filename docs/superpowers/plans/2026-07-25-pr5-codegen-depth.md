...
**Historical Task 6 scope note on `None`:** the original brainstorm scope named this task "`float`/`bool`/`None` runtime representation." At Task 6, repo research found no bare `None` value to build: `pycc_hir::HirExpr` has no `NoneLiteral` variant (`annotation_to_ty` handles `Expr::NoneLiteral` only in *type-annotation* position, e.g. `def f(x: None) -> None`, and `lower_expr` rejects a bare `None` used as a value). The plan therefore deferred the then-reachable direct-call result to Task 10's print dispatch and implemented only `float` and `bool` promotion here. D-075 later superseded that direct-call-only boundary for parameter-carried `None`: a `None` parameter name can now flow through another call, `return`, `print`, and f-string interpolation. See Task 10's updated scope note below; D-072 still excludes using `print()` itself as a nested expression.
...
**Scope note on `None` (historical Task 10 plan, superseded in part by D-075):** this task originally handled only a direct `None`-returning call such as `print(f())`. PR #132's review later found that the already-accepted `def sink(value: None) -> None: ...` signature still failed during codegen declaration. D-075 adds the canonical `i8 0` user-function parameter carrier plus name reads from parameter entry slots, so `print(value)`, f-string interpolation, and `return value` now work for a `None`-typed parameter. D-072 still excludes using `print()` itself as a nested expression, and general `None` assignment storage remains outside this plan.
