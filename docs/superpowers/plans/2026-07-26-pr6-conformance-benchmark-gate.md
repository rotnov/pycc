...
```
`actions/setup-python` only puts a plain `python.exe` (not `python3.14`) on `PATH` on Windows — the `Alias python3.14` step above copies it to a `python3.14.exe` sibling. Resolve the source directory via `$pythonLocation` (the exact directory `actions/setup-python` itself exports and prepends to the real Windows `PATH`), converted to a POSIX path with `cygpath -u` for bash's `cp` — **not** bash's own `command -v python`, which was tried first and failed in real CI: Git Bash's own `PATH` view can be reordered relative to the real Windows `PATH` that later `pwsh`-run steps (and the Rust test binary's own `Command::new` PATH search) actually search, so a bash-resolved "python" is not guaranteed to be the interpreter those later steps will find. This must run before `Verify oracle version`, since that step checks `python3.14` unconditionally on every OS. Then change the job's existing `cargo test --workspace` step(s) — both the non-Windows and the `--test-threads=1` Windows variant — to add `-- --include-ignored` (the Windows variant becomes `-- --test-threads=1 --include-ignored`), since `tests/conformance.rs`'s two tests are `#[ignore]`d by default and need that flag to actually execute.

**Second post-hoc correction (D-080 addendum):** the alias step above is *necessary but not sufficient*. Real CI evidence showed the Windows leg still failing with "program not found" after the alias was in place and confirmed working (bash's own "Verify oracle version" step ran `python3.14 --version` successfully against that exact alias). The remaining gap is on the Rust side: `std::process::Command::new` on Windows resolves a bare program name by searching `PATH` for that literal file name and does **not** append `.exe` when the name already contains a `.` — the version dot in `"python3.14"` reads as an extension marker to that resolver, unlike bash's own exec resolution, which does try appending `.exe`. Same file, same `PATH`, two different resolvers. So `tests/conformance.rs::oracle_python_bin` must also request `"python3.14.exe"` explicitly under `cfg!(windows)` (and plain `"python3.14"` elsewhere) — this was originally rejected in D-080's Alternatives as an unnecessary platform special-case, but real CI evidence overrides that: the CI-side alias alone cannot make D-085's (originally recorded as D-078, later renumbered — see the Global Constraints note above) single-hardcoded-name ideal survive Windows. Keep the alias step exactly as written above; only `tests/conformance.rs`'s lookup needs the platform-conditional extension.
...
Read it once more end-to-end; confirm nothing this PR changed (D-079/D-080/D-082/D-083/D-084/D-085 — D-078/D-081 belong to unrelated concurrent `main` PRs, not this one — plus the corrected CLI_SPEC.md example) contradicts anything it currently says. It shouldn't — this PR added test/CI infrastructure and one docs correction, not new language-surface behavior.
...
Review each hit; fix any that still describes pre-PR-6 state (e.g. `docs/SPEC.md`'s `pycc_testkit` mention should stay as-is per D-085's Consequences — it correctly describes a still-planned crate, not something to remove).
