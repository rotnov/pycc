...
**Correction found after Task 10's own execution — Step 5 reverted, not just fixed:** `.github/workflows/ci.yml` is a whole-file digest-pinned trust anchor (`scripts/check_roadmap_evidence.rb`'s D-100 composed-workflow check) — this plan's own file-list note above ("comments only... no functional change") is true about runtime behavior but does not exempt the file from the pin, which hashes exact bytes regardless of whether a byte-diff is functional. Editing the comments (first to "11", then a same-day follow-up fix to "12" after the pinned reviewer caught the undercount) broke the pin both times, failing `audit` and `build-test-coverage`'s `test_d100_composed_workflow_is_active_and_reviewed` on the pushed PR. Fix: both edits were reverted (`git checkout origin/main -- .github/workflows/ci.yml`), restoring the exact pinned blob (verified via `git rev-parse` blob-hash equality with `origin/main` and a clean `ruby scripts/check_roadmap_evidence.rb` + `ruby scripts/test_check_roadmap_evidence.rb` pass, 131 runs/0 failures). The stale "two"/"Both" comment counts remain in `ci.yml` as shipped — this is now a **known, deliberately deferred** cosmetic gap, to be fixed only by a future PR that already legitimately re-stages `ci.yml`'s digest for some functional reason (per D-090/091/092/099/100's stage-then-activate pattern), not by a comment-only edit on its own. Lesson for future tasks: any planned "comment-only, no functional change" edit to `.github/workflows/ci.yml` still needs to route through the digest re-pin process first — there is no small-enough edit that is exempt.
