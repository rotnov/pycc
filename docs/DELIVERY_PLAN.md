...
gate is now in D-051's bootstrap-free steady state. Every pull request measures
the frontend; the isolated comparison job accepts only the non-expired
artifact from the exact successful `main` predecessor, fails closed when that
artifact is unavailable, and blocks `ci-gate` on a regression greater than 2%.
The one-time activation variable, transition fixtures, digests, and bootstrap
tests are retired. PR-5 is no longer blocked on performance-gate activation.
