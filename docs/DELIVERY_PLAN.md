...
Every ROADMAP.md milestone is its own sub-project: its own brainstorm → plan → implementation cycle, entered only when the previous milestone's acceptance criteria are green. v0.1 is fully detailed below (see [Delivery mechanics](#delivery-mechanics) for why). v0.2's brainstorm phase is this decision (D-088) plus its companion architecture decision; its own execution-strategy and PR-breakdown section is added to this file in a companion PR as that phase completes, not deferred indefinitely. v0.3+ get the same treatment when reached, since their design will be shaped by what v0.1–v0.4 teach us.
