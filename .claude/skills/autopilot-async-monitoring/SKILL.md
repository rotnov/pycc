...
## Use `gha-watch-ci-pr`'s `ci-watch.sh` + `Monitor` instead of a fixed `ScheduleWakeup` interval
...
Instead, run the `gha-watch-ci-pr` skill's `ci-watch.sh <repo> <pr-number>
[<pr-number> ...]` via the `Monitor` tool (`persistent: false`, a generous
`timeout_ms` — the script exits on its own once every listed PR reaches a
terminal state, so the timeout is just a backstop). The script polls every
`POLL_INTERVAL` seconds (default 10, overridable via env) and prints exactly
one line per PR the moment it becomes: `MERGED`/`CLOSED`, `CONFLICTS` (merge
base diverged), `STALE` (branch fell behind base — e.g. a sibling PR merged
first), `CHECK FAILED -- <name> (<conclusion>)[, <name>
(<conclusion>)...]` (every non-passing check is named, not just the first),
`BLOCKED` (every check completed with none failing, but
`mergeStateStatus` is something other than `CLEAN`/`BEHIND` — typically an
...
are reported only after the same verdict holds on two consecutive polls, so
...
GitHub PR), the same pattern — a poll loop that emits one line per
