...
//! directory (13). Nothing here touches the file system or spawns anything,
//! so the macOS coverage host drives every line over synthetic byte
//! fixtures; `native_windows.rs` does the walking.
//!
...
pub(crate) fn parse_pe(bytes: &[u8]) -> Result<Option<PeImage>, String> {
    ...
    let directory = |index: usize| match index < dir_count {
        true => u32_at(bytes, optional + dirs_off + index * 8).unwrap_or_default(),
        false => 0,
    };
    let imports = names(bytes, &mapped, directory(IMPORT_DIRECTORY), false)?;
    let delay_imports = names(bytes, &mapped, directory(DELAY_IMPORT_DIRECTORY), true)?;
    Ok(Some(PeImage {
        machine,
        pe32_plus,
