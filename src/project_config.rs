...
"#;
    ...
    fn chmod_containing_dir_readonly_then_fail_write(
        ...
        //
        // This is why this pair of call sites (here and in
        // `write_new_in_reports_a_cleanup_failure_honestly` below) is not
        // migrated onto `ScratchDir` as part of issue #782's Batch B:
        // `ScratchDir::new`'s directory name embeds a per-call nanosecond
        // timestamp and atomic sequence number, so it cannot be recomputed
        // from process state alone by a capture-free `fn` pointer with no
        // shared context. The deterministic `pid`-only path this pair
        // relies on stays as a raw `std::env::temp_dir()` call.
    ...
    fn scaffold_fails_cleanly_when_src_is_read_only() {
        ...
        // Restore write permission before the scratch dir's Drop tries to
        // remove `src` and its contents.
