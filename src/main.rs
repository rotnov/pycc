...
mod try_build_release_isolation_tests {
    ...
    fn try_build_ignores_a_neighboring_release_pycc_toml_when_given_release_false() {
        ...
        // Not migrated onto `ScratchDir`: this must resolve to the exact
        // path `try_build`'s own production code (line 203 above, out of
        // scope for issue #782's Batch B -- owned by #783) hardcodes for
        // its temp object file, keyed only by this test process's pid.
        let obj_path = std::env::temp_dir().join(format!("pycc_obj_{}.o", std::process::id()));
