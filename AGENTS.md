...
- Do not pause a task to ask the repository owner a clarifying question, request approval, or ask them to pick between alternatives. When a task reaches a genuine fork in judgment (a design tradeoff, a policy interpretation, a risk call, whether a finding is real), resolve it yourself by consulting a stronger independent reviewer (this session's own advisor/second-opinion tool, or an independent review agent) rather than soliciting the repository owner. Record the reasoning and the resolution in the relevant ADR, plan, or session log exactly as you would any other decision. That consultation *is* the resolution, so report it as a decision made: state the call and the reasoning behind it. Do not describe a fork as one the agent may not settle on its own, as awaiting the owner, or as open pending their input — a report phrased that way misstates the operating model even when the action taken complied with it, and invites an intervention this model does not require.
...
- One writer per worktree. A dispatched subagent that has returned a report may still be running; a report is not a termination. Before this session reads, edits, debugs, or runs a gate against files a dispatched agent was given, enumerate live background tasks and terminate any that share the worktree. While two writers share a tree, every gate verdict taken in that window is void — including green ones — so re-run the full gate set from a single-writer baseline rather than trusting results collected during the overlap.
...
- Record irreversible or project-wide design choices as a new file under `docs/decisions/` (see `docs/decisions/TEMPLATE.md`). The bar ([D-242](docs/decisions/D-242-product-mode-the-delivery-process-informs-rather-than-blocks.md) rule 3): irreversible or project-wide means ABI and artifact formats, semantics that deliberately deviate from CPython, security and trust boundaries, and the operating model itself; registering a stdlib symbol, adding a builtin method, naming a scratch directory, choosing a test layout, or fixing a gate is a pull request with tests and, where a rule changes, an `AGENTS.md` or owning-specification edit — not a decision file. After adding one, regenerate `docs/decisions/README.md` with `python3 scripts/generate_decisions_index.py docs/decisions docs/decisions/README.md` (append `--check` to verify freshness without rewriting; CI's `governance` job runs that `--check` on every pull request, so a stale index or a duplicate `D-NNN` id fails `ci-gate`). Do not silently rewrite an accepted decision; add a new decision that supersedes it. CI's `governance` job enforces this with `scripts/check_decision_immutability.py` ([D-240](docs/decisions/D-240-accepted-decision-files-are-insert-only-and-ci-enforces-it.md)): once a decision file's frontmatter says `accepted` or `superseded`, every one of its existing lines must survive verbatim and in order. The only permitted changes are inserted lines (a dated amendment such as `- Amendment (YYYY-MM-DD): ...`, which is also how a typo is corrected), replacing the frontmatter `status:` line and the first body `- Status:` line to record a transition to `superseded` or a narrowing annotation (never back to `accepted` or `proposed`), and replacing the five stub body lines of a D-151 index-only stub with its long-form entry. Deleting or renaming such a file is a violation. Run `python3 -B scripts/check_decision_immutability.py --base "$(git merge-base origin/main HEAD)" --head HEAD` locally before pushing a change under `docs/decisions/`.
...
## Testing and the coverage gate

- One hundred percent line coverage of the Rust lines a compiler-relevant pull request adds or modifies is a hard merge invariant under [D-242](docs/decisions/D-242-product-mode-the-delivery-process-informs-rather-than-blocks.md) (narrowing D-014), not a target or guideline. It is measured by `scripts/check_diff_coverage.py` from the `cargo llvm-cov --workspace --lcov` export joined against a zero-context diff of the pull request's own changes; a changed instrumentable file with no coverage record fails the gate closed. Total workspace line coverage and the per-crate region table are reported in the same job, never enforced: coverage lost on unchanged lines (a deleted test) is visible only in that report, and the D-068 review reads it.
- CI must run `run_isolated "$TRUSTED_COV" llvm-cov --workspace --lcov --output-path "$ISOLATED_ROOT/coverage.lcov"`, `run_isolated "$TRUSTED_COV" llvm-cov report "${WORKSPACE_PACKAGE_FLAGS[@]}"`, and `python3 -B scripts/check_diff_coverage.py --lcov "$ISOLATED_ROOT/coverage.lcov" --diff "$RUNNER_TEMP/coverage-changed.diff" --root "$GITHUB_WORKSPACE" --require-changed-lines 100` for every compiler-relevant pull request selected by the fail-closed classifier and every push to `main`, the `llvm-cov` invocations inside the checker-approved, sanitized `nobody` boundary and without an earlier head-controlled script. Do not merge while a selected coverage gate is missing, skipped, cancelled, failing, or still in progress.
...
- Every CI job that runs the test suite runs it with `-- --include-ignored` (`.github/workflows/ci.yml`). A local run without that flag is therefore a different gate, not a cheaper form of the same one: a test the diff itself marks `#[ignore]` is green locally and red in CI. Run the local test gate with the flag and attribute each failure before merging -- some `#[ignore]`d tests are environment-gated (an oracle interpreter pin, development headers) and fail for reasons the diff does not own.
- Never lower the `--require-changed-lines` threshold, drop the gate command or the `--workspace` denominator, disable the job, or exclude code merely to make a pull request pass.
...
- Regular CI must run `ruby scripts/check_ci_permissions.rb` for fast feedback. The read-only `Workflow policy` check (`audit`) is the trust anchor: it runs on every pull request from the base commit under `pull_request_target`, never checks out or executes pull-request code, and audits the head revision's workflow YAML plus roadmap acceptance evidence as data. Under D-172 it validates named permissions, immutable Action references, checkout credentials, change routing, Tier-1 coverage, and aggregate-gate properties instead of requiring general CI/checker files to equal predecessor-staged whole-file bytes. Keep that check required before merging -- never permanently remove or downgrade it. The one narrow exception is D-125's session-driven temporary bypass, which may relax `audit` for exactly one publicly tracked, expiry-bound incident at a time and restores it immediately afterward; see `docs/REPOSITORY_GOVERNANCE.md`'s "Session-driven temporary bypass" section.
- Standing owner authorization (2026-08-14): the repository owner permits the agent to invoke the D-125 session-driven temporary bypass for `audit` check failures without asking for per-instance permission, provided the agent first confirms through the full Gate 1 adversarial verification that the failure is not attributable to the PR's own diff. This covers the recurring D-103 manifest deadlock class and any other audit failure where the PR's own content is not the cause. The agent must still follow every step of the `ci-temporary-bypass` skill workflow (Gate 1, relax, merge, restore, Gate 2) and must never skip the restore or Gate 2 verification. This authorization does not extend to any other required check, does not waive the scope-boundary conditions, and does not authorize permanent removal of `audit` from branch protection.
- Recovery-only authorization (2026-08-17): because the #558 diff itself retires D-103's deadlocking exact-byte gate, D-125 is ineligible for that candidate-caused failure. The owner authorized one D-024 operation for the single #558 recovery PR: relax only `audit`, merge at most the exact reviewed head within ten minutes, never relax `ci-gate`, restore the exact protection snapshot immediately, and complete independent post-restore verification. This is not standing authority and cannot be reused for another pull request.
...
- A Rust source file over ~1,000 lines is a maintainability and agent-context risk (the largest files here still run to 11-25k lines, past what a single `Read` call covers). When a task's own work touches such a file, decompose the part it touches into cohesion-driven submodules as part of that same change — not as a separate dedicated refactor task, and not by rewriting unrelated code.
...
- Keep required status checks, including the coverage gate, and required conversation resolution enabled. Revisit the approving-review requirement when a second human maintainer is available.
...
- Address every verified P0/P1 finding and every other actionable correctness
  ...
  the current diff.
- Merge only when required checks, including the coverage gate, are
...
## Keep a retrospective log and a session handoff log ([D-066](docs/decisions/D-066-maintain-an-agent-retrospective-log-and-a-session.md), [D-130](docs/decisions/D-130-decompose-the-session-handoff-log-into-per.md), [D-192](docs/decisions/D-192-bound-the-tracker-with-milestone-at-filing-a.md))
...
- `docs/sessions/` holds a handoff snapshot only for an **incident** — a wrong merge decision, a defect that reached `main`, a gate that lied, or a run that lost meaningful time to a process mistake ([D-242](docs/decisions/D-242-product-mode-the-delivery-process-informs-rather-than-blocks.md) rule 5 narrows D-066/D-130/D-192 here; the same definition governs `.harden/findings/` piles, `harden batch` passes, and `.harden/incidents/` entries). An ordinary merged pull request writes none of them. When an incident does call for one, write it in the pull request that delivers the work, so it lands with that merge — a new file named `YYYY-MM-DD-NN-<slug>.md` (extending the `docs/superpowers/specs`/`docs/superpowers/plans` dated-slug convention with a two-digit same-day sequence number — `01` for the day's first entry, ascending for later ones, since several dates already carry many same-day checkpoints and the date prefix alone cannot order them) with overall status, what's currently in flight, known follow-ups, and where a fresh session should look to resume. Ground each snapshot in the exact commit and repository state actually inspected, and distinguish uncommitted or unmerged work from delivered work. Never edit a previous session's file to append a later snapshot — a new snapshot is always a new file, so two sessions active at once never race to edit the same one. `docs/sessions/README.md` is the sole exception: a static purpose statement, not an entry log, never appended to. A task without an incident writes no session file, and an incident writes exactly one: everything else the old per-checkpoint and per-merge triggers would have captured — a fix round, an intermediate CI result, a lesson learned — belongs in `docs/AGENT_RETROSPECTIVE.md` under its unchanged bar, or nowhere.
...
6. When a task is given as a list or collection of items (a file of issues, a set of bug reports, a milestone's issue set), the todo list must include every item as a pending entry before starting work on the first one. A plan's own enumerated non-code deliverables are a list of this kind: a clause directing that a follow-up issue be filed, a measurement be recorded, or a named document be updated is an item to track, not prose accompanying the code changes. Implementing every code change a plan specifies does not discharge the clauses that specify something other than code. The overall task is not complete until every item is either done, explicitly deferred with a recorded reason, or determined to be no longer actionable with cited evidence. Completing one item and marking the entire task done is a false-completion error — the todo list is the guard, and it must reflect the full scope, not just the first item.
