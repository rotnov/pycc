...
- This does not relax any other rule in this document or in an agent's own safety boundaries. Actions that require a human administrator elsewhere in this file (the [D-024](docs/DECISIONS.md#d-024-protected-main-and-audited-emergency-bypass) emergency path, credential and account actions, and any other action a session's own safety rules reserve for the user) still require the repository owner to act, and an agent still asks before taking them — this section only removes the expectation of asking before ordinary engineering judgment calls.
- To conserve tokens across a long autonomous run, isolate a task's heavy work (plan authoring, implementation, iterative fix loops) inside dispatched `Agent` calls rather than doing it directly in this session's own context (see [D-142](docs/DECISIONS.md#d-142-issue-implements-step-4-implementation-runs-in-a-dispatched-agent-not-the-orchestrating-sessions-own-context) for implementation/fix-loop dispatch and [D-143](docs/DECISIONS.md#d-143-issue-implements-delegated-issue-to-plan-invocation-also-runs-inside-a-dispatched-agent) for its extension to `issue-to-plan`'s own plan-authoring work) — this is the mechanism actually available for bounding context growth at task checkpoints. On-demand session compaction (compacting *now*, at a moment of the agent's own choosing, such as right after a merge) is not available from inside a running session: no hook can initiate it, and there is no `settings.json` field, CLI flag, or SDK call that triggers an immediate compaction — confirmed directly against Claude Code's own hooks and Agent SDK documentation (the SDK's `Query` control surface has no `compact`-equivalent method), not merely assumed (see [D-127](docs/DECISIONS.md#d-127-autonomous-agent-operation-model)'s own correction). This is distinct from the auto-compact *threshold*, which the `autoCompactWindow` setting genuinely does control (see `.claude/settings.json`, currently `250000`): lowering it makes automatic compaction fire sooner, but the moment it fires is still whatever point the running turn happens to be at when the threshold is crossed, not a checkpoint the agent picks. A genuinely fresh session per checkpoint would reset context fully, but starting one requires the repository owner or an external process to initiate it — an agent cannot restart itself into a clean session.
...
## Keep a retrospective log and a session handoff log ([D-066](docs/DECISIONS.md#d-066-maintain-an-agent-retrospective-log-and-a-session-handoff-log), [D-130](docs/DECISIONS.md#d-130-decompose-the-session-handoff-log-into-per-session-files))
