...
- To conserve tokens across a long autonomous run, isolate a task's heavy work (plan authoring, implementation, iterative fix loops) inside dispatched `Agent` calls rather than doing it directly in this session's own context (see [D-142](docs/DECISIONS.md#d-142-issue-implements-step-4-implementation-runs-in-a-dispatched-agent-not-the-orchestrating-sessions-own-context) for implementation/fix-loop dispatch and [D-143](docs/DECISIONS.md#d-143-issue-implements-delegated-issue-to-plan-invocation-also-runs-inside-a-dispatched-agent) for its extension to `issue-to-plan`'s own plan-authoring work) — this is the mechanism actually available for bounding context growth at task checkpoints. Self-triggered session compaction (`/compact`/`/clear`) is not available from inside a running session: no hook can initiate it, and there is no `settings.json` field, CLI flag, or SDK call for a non-interactive equivalent — confirmed directly against Claude Code's own hooks documentation, not merely assumed (see [D-127](docs/DECISIONS.md#d-127-autonomous-agent-operation-model)'s own correction). A genuinely fresh session per checkpoint would reset context fully, but starting one requires the repository owner or an external process to initiate it — an agent cannot restart itself into a clean session.
