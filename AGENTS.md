...
- CI must run `run_isolated "$TRUSTED_COV" llvm-cov --workspace --fail-under-lines 100 --fail-under-regions 100` for every compiler-relevant pull request selected by the fail-closed classifier and every push to `main`, inside the checker-approved, sanitized `nobody` boundary and without an earlier head-controlled script. Do not merge while a selected coverage gate is missing, skipped, cancelled, failing, or still in progress. The activation pull request for #1010 replaces this command line with `run_isolated "$TRUSTED_COV" llvm-cov --workspace --lcov --output-path "$ISOLATED_ROOT/coverage.lcov"`, `run_isolated "$TRUSTED_COV" llvm-cov report "${WORKSPACE_PACKAGE_FLAGS[@]}"`, and `python3 -B scripts/check_diff_coverage.py … --require-changed-lines 100`; until it lands, `scripts/check_roadmap_evidence.rb` accepts both shapes.
