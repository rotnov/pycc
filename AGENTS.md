...
10. Before writing implementation code for a multi-step or architecturally consequential task, confirm a written plan exists or produce one — this applies regardless of the task's source, not only GitHub issues. `.claude/skills/issue-to-plan/SKILL.md` is scoped to a single named issue by design (see its own Scope section) and does not cover roadmap/milestone-decomposition work (a `docs/DELIVERY_PLAN.md` PR-N task, a `docs/DECISIONS.md` ADR follow-up, a CI-gate fix round). For that work, treat the matching section of `docs/DELIVERY_PLAN.md` as the plan when one already exists at the needed level of detail, or produce one first via `superpowers:writing-plans`. A small, single-file, mechanically-scoped fix (a docs correction, a one-line CI-permission fix) does not need this step — use judgment on "multi-step or architecturally consequential" rather than treating every change as requiring a formal plan.
