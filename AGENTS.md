...
- CI must run `run_isolated "$TRUSTED_COV" llvm-cov --workspace --fail-under-lines 100 --fail-under-regions 100` for every compiler-relevant pull request selected by the fail-closed classifier and every push to `main`, inside the checker-approved, sanitized `nobody` boundary and without an earlier head-controlled script. Do not merge while a selected coverage gate is missing, skipped, cancelled, failing, or still in progress.
...
- `docs/sessions/` holds one handoff snapshot per **merged pull request**, not a shared transcript and no longer one per meaningful checkpoint ([D-192](docs/decisions/D-192-bound-the-tracker-with-milestone-at-filing-a.md) narrows D-066/D-130 here): write it in the pull request that delivers the work, so it lands with that merge — a new file named `YYYY-MM-DD-NN-<slug>.md` (extending the `docs/superpowers/specs`/`docs/superpowers/plans` dated-slug convention with a two-digit same-day sequence number — `01` for the day's first entry, ascending for later ones, since several dates already carry many same-day checkpoints and the date prefix alone cannot order them) with overall status, what's currently in flight, known follow-ups, and where a fresh session should look to resume. Ground each snapshot in the exact commit and repository state actually inspected, and distinguish uncommitted or unmerged work from delivered work. Never edit a previous session's file to append a later snapshot — a new snapshot is always a new file, so two sessions active at once never race to edit the same one. `docs/sessions/README.md` is the sole exception: a static purpose statement, not an entry log, never appended to. A task that merges nothing writes no session file, and a task that merges one pull request writes exactly one: every other thing the old per-checkpoint trigger would have captured mid-task — a fix round, an intermediate CI result, a lesson learned — belongs in `docs/AGENT_RETROSPECTIVE.md` instead.
