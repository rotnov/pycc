...
- Regular CI must run `ruby scripts/check_ci_permissions.rb` for fast feedback. The read-only `Workflow policy` check is the trust anchor: it runs on every pull request from the base commit under `pull_request_target`, never checks out or executes pull-request code, and audits the head revision's workflow YAML, roadmap acceptance evidence, and projected search-history checkpoint retention as data. Head-controlled CI must separately bind those checkpoint markers to the search ledger. Keep that check required before merging.
