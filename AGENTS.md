...
- Establish an explicit monitoring checkpoint from the refreshed remote default-branch commit, the current open pull-request inventory, and the current state of every task-active pull request. Report that commit when monitoring starts or resumes.
- After the checkpoint, monitor only a newly observed default-branch commit; a pull request that was opened, reopened, or updated; or a state, head, review-thread, or required-check change on a pull request already active in the current task.
