...
- CI must run `/Users/runner/.cargo/bin/cargo-llvm-cov llvm-cov --workspace --fail-under-lines 100 --fail-under-regions 100` on every pull request, after the checker-approved setup prefix and without an earlier head-controlled script. Do not merge while this gate is missing, skipped, cancelled, failing, or still in progress.
